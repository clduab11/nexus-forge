...
class MarketplaceSearchEngine:
    ...
    async def get_trending(
        ...
        query = self.supabase.client.table("agent_packages") \
            .select("*") \
        ...
        for agent in agents:
            ...
            agent.trending_score = (
                recent_downloads * 0.7 +
                recent_ratings * 10 * 0.3
            )
        ...
        await self.cache.set(
            cache_key,
            [agent.dict() for agent in trending],
            ttl=3600  # 1 hour
        )
    ...
    async def _semantic_search(
        ...
        db_query = self.supabase.client.table("agent_packages") \
            .select("*") \
            .eq("status", AgentStatus.APPROVED.value)
    ...
    def _apply_filters(
        ...
        if "license" in filters:
            filtered = [
                a for a in filtered
                if a.manifest.license == filters["license"]
            ]
    ...
    def _generate_cache_key(
        ...
        limit: int, offset: int, sort_by: str
    ) -> str:
    ...
    async def _get_tag_suggestions(
        ...
        common_tags = [
            "ai", "nlp", "vision", "audio", "automation",
            "data", "analytics", "integration", "utility", "workflow"
        ]
    ...
    async def _get_recent_downloads(
        self, agent_id: str, since: datetime
    ) -> int:
        """Get download count since date"""
        query = self.supabase.client.table("agent_installations") \
    ...
    async def _get_recent_ratings(
        ...
        query = self.supabase.client.table("agent_ratings") \
            .select("id", count="exact") \
            .eq("agent_id", agent_id) \
    ...
    async def _get_agent_by_id(self, agent_id: str) -> Optional[AgentPackage]:
        """Get agent by ID"""
        result = await self.supabase.client.table("agent_packages") \
            .select("*") \
            .eq("id", agent_id) \
            .execute()
    ...
    def _calculate_similarity(
        ...
        if agent1.downloads and agent2.downloads:
            pop_ratio = min(agent1.downloads, agent2.downloads) / max(agent1.downloads, agent2.downloads)
            score += 0.1 * pop_ratio
