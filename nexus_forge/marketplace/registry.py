...
from .models import (
    AgentPackage, AgentManifest, AgentStatus, 
    SecurityReport, PerformanceMetrics
)
...
class AgentRegistry:
    ...
    async def rate_agent(
        ...
        existing = await self.supabase.client.table("agent_ratings").select("*") \
    ...
    async def _validate_manifest(self, manifest: AgentManifest) -> None:
        ...
        existing = await self.supabase.client.table("agent_packages") \
            .select("id") \
    ...
    async def _check_dependencies(self, dependencies: List[Dict[str, str]]) -> None:
        ...
        for dep in dependencies:
            ...
            try:
                ...
                raise ValidationError(
                    f"Dependency {dep_name}@{dep_version} not found"
                )
    ...
    async def _update_agent_rating(self, agent_id: str) -> None:
        ...
        result = await self.supabase.client.table("agent_ratings") \
            .select("rating") \
            .eq("agent_id", agent_id) \
            .execute()
