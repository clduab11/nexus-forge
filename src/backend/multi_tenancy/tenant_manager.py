...
class TenantManager:
    ...
    async def create_tenant(
        ...
        tenant = Tenant(
            ...
            tier=tier,
            status=(
                TenantStatus.TRIAL if tier == TenantTier.BASIC else TenantStatus.ACTIVE
            ),
    ...
    async def update_tenant(self, tenant: Tenant) -> Tenant:
        ...
        # Log update
        await self._log_audit_event(
            tenant.id, "tenant_updated", "tenant", f"Tenant {tenant.name} updated"
        )
    ...
    async def delete_tenant(self, tenant_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            await self.supabase.client.table("tenants").delete().eq(
                "id", tenant_id
            ).execute()
    ...
    async def remove_user(self, tenant_id: str, user_id: str, removed_by: str) -> bool:
        """Remove a user from a tenant"""
        # Remove tenant user association
        await self.supabase.client.table("tenant_users").delete().eq(
            "tenant_id", tenant_id
    ...
    async def update_user_role(
        ...
        # Update role
        await self.supabase.client.table("tenant_users").update({"role": new_role}).eq(
            "tenant_id", tenant_id
        ).eq("user_id", user_id).execute()
    ...
    async def update_resource_usage(self, tenant_id: str, usage: ResourceUsage) -> None:
        """Update tenant resource usage"""
        usage.last_updated = datetime.utcnow()

        # Update in database
        await self.supabase.client.table("tenants").update({"usage": usage.dict()}).eq(
            "id", tenant_id
        ).execute()
        ...
        if resource_type == "cpu_cores":
            if tenant.quota.cpu_cores:
                ...
                if requested_amount > available:
                    return (
                        False,
                        f"CPU limit exceeded. Available: {available:.2f} cores",
                    )
            ...
            if tenant.quota.memory_gb:
                ...
                if requested_amount > available:
                    return (
                        False,
                        f"Memory limit exceeded. Available: {available:.2f} GB",
                    )
            ...
            if tenant.quota.storage_gb:
                ...
                if requested_amount > available:
                    return (
                        False,
                        f"Storage limit exceeded. Available: {available:.2f} GB",
                    )

        elif resource_type == "agents":
            if tenant.quota.max_agents:
                if (
                    tenant.usage.current_agents + requested_amount
                    > tenant.quota.max_agents
                ):
                    return (
                        False,
                        f"Agent limit exceeded. Limit: {tenant.quota.max_agents}",
                    )

        elif resource_type == "workflows":
            if tenant.quota.max_workflows:
                if (
                    tenant.usage.current_workflows + requested_amount
                    > tenant.quota.max_workflows
                ):
                    return (
                        False,
                        f"Workflow limit exceeded. Limit: {tenant.quota.max_workflows}",
                    )

        elif resource_type == "users":
            if tenant.quota.max_users:
                if (
                    tenant.usage.current_users + requested_amount
                    > tenant.quota.max_users
                ):
                    return (
                        False,
                        f"User limit exceeded. Limit: {tenant.quota.max_users}",
                    )
    ...
    async def get_tenant_usage_report(self, tenant_id: str) -> Dict[str, Any]:
        ...
        if tenant.quota.cpu_cores:
            usage_percentages["cpu"] = (
                tenant.usage.cpu_usage / tenant.quota.cpu_cores
            ) * 100

        if tenant.quota.memory_gb:
            usage_percentages["memory"] = (
                tenant.usage.memory_usage_gb / tenant.quota.memory_gb
            ) * 100

        if tenant.quota.storage_gb:
            usage_percentages["storage"] = (
                tenant.usage.storage_usage_gb / tenant.quota.storage_gb
            ) * 100
    ...
    async def get_audit_logs(
        ...
        query = (
            self.supabase.client.table("tenant_audit_logs")
            .select("*")
    ...
    async def _save_tenant(self, tenant: Tenant) -> None:
        """Save tenant to database"""
        await self.supabase.client.table("tenants").insert(tenant.dict()).execute()

    async def _update_tenant(self, tenant: Tenant) -> None:
        """Update tenant in database"""
        await self.supabase.client.table("tenants").update(tenant.dict()).eq(
            "id", tenant.id
        ).execute()
    ...
    async def _update_invitation(self, invitation: TenantInvitation) -> None:
        ...
        await self.supabase.client.table("tenant_invitations").update(
            invitation.dict()
    ...
    async def _send_invitation_email(
        ...
        logger.info(
            f"Sending invitation email to {invitation.email} for tenant {tenant.name}"
        )

        # Email would contain invitation link with token
        invitation_url = (
            f"https://app.nexusforge.ai/invite/{invitation.invitation_token}"
        )
