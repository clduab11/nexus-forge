...
class CollaborativeResearchManager:
    ...
    def __init__(
        ...
        self.user_presence: Dict[str, Set[str]] = (
            {}
        )  # collab_id -> set of active user_ids
    ...
    async def create_collaboration(
        ...
        # Store in Firestore
        await self.state.firestore.collection("collaborations").document(collab_id).set(
            asdict(collaboration)
        )
    ...
    async def join_collaboration(
        self, collab_id: str, user_id: str, invite_code: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if collaboration.settings.get("share_mode") == "private":
            if not invite_code or not await self._validate_invite_code(
                collab_id, invite_code
            ):
                return {"success": False, "error": "Valid invite code required"}
        ...
        new_member = CollaborationMember(
            user_id=user_id, role=role, joined_at=now, last_active=now
        )
        ...
        await self.state.firestore.collection("collaborations").document(
            ...
            {"members": firestore.ArrayUnion([asdict(new_member)]), "updated_at": now}
        )
        ...
        # Notify all members
        await self._broadcast_to_collaboration(
            collab_id,
            {
                "type": "member_joined",
                "user_id": user_id,
    ...
    async def share_research(
        self, collab_id: str, user_id: str, task_id: str, include_graph: bool = True
    ) -> bool:
        """
        ...
        if not await self._check_permission(
            collab_id, user_id, CollaborationPermission.CREATE_RESEARCH
        ):
            return False
    ...
    async def coordinate_research(
        ...
        active_members = [
            m
            for m in collaboration.members
        ...
        for i, subtask in enumerate(subtasks):
            assigned_user = (
                assigned_users[i % len(assigned_users)] if assigned_users else None
            )
    ...
    async def merge_knowledge_graphs(
        self, collab_id: str, graph_ids: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for graph_id in graph_ids:
            graph_data = next(
                (g for g in collaboration.shared_graphs if g["id"] == graph_id), None
            )
            if graph_data:
    ...
    async def get_collaboration_analytics(self, collab_id: str) -> Dict[str, Any]:
        """
        ...
        analytics = {
            ...
            "most_active_member": (
                max(member_stats, key=lambda x: x["contributions"])
                if member_stats
                else None
            ),
    ...
    async def _get_collaboration(
        ...
        # Load from Firestore
        doc = (
            await self.state.firestore.collection("collaborations")
            .document(collab_id)
            .get()
        )
    ...
    async def _check_permission(
        self, collab_id: str, user_id: str, permission: CollaborationPermission
    ) -> bool:
    ...
    async def _broadcast_to_collaboration(
        self, collab_id: str, message: Dict[str, Any]
    ):
    ...
    async def _validate_invite_code(self, collab_id: str, invite_code: str) -> bool:
        ...
        doc = (
            await self.state.firestore.collection("invite_codes")
    ...
    async def _get_activity_timeline(
        ...
        query = (
            self.state.firestore.collection("shared_research")
            .where("collaboration_id", "==", collab_id)
            .where("shared_at", ">=", start_date.isoformat())
