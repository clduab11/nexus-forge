...
class RetrievalAgent(Agent):
    ...
    async def search_information(
        self, query: str, max_sources: int = 10, force_refresh: bool = False
    ) -> List[Source]:
        """
    ...
    async def _perform_search(
        ...
        try:
            ...
            if GOOGLE_SEARCH_AVAILABLE and google_search_tool:
                ...
                search_result = await google_search_tool.search(
            ...
            results = await loop.run_in_executor(
                None, lambda: self.searcher.search(query, num_results=max_results)
            )
            return results
    ...
    def _calculate_reliability(self, source: Source) -> float:
        ...
        if any(
            ...
            for indicator in ["/pdf/", ".pdf", "doi.org", "research"]
        ):
            base_score += 0.1
    ...
    async def enhanced_search(
        self, query: str, max_sources: int = 10, depth_level: str = "detailed"
    ) -> List[Source]:
        """
        ...
        """
        self.logger.info(
            f"Performing enhanced search for: {query} (depth: {depth_level})"
        )
    ...
    def get_source_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_reliability_domains": len(
                [d for d, s in self.domain_reliability.items() if s > 0.8]
            ),
