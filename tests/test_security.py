...
class TestResearchQueryValidator:
    ...
    def test_focus_areas_sanitization(self):
        ...
        validator = ResearchQueryValidator(
            query="Test", focus_areas=["science", "technology", "medicine"]
        )
        assert len(validator.focus_areas) == 3

        # With HTML attempts
        validator = ResearchQueryValidator(
            query="Test", focus_areas=["<b>science</b>", "tech<script>alert()</script>"]
        )
        ...
        with pytest.raises(ValidationError):
            ResearchQueryValidator(
                query="Test", focus_areas=["area" + str(i) for i in range(11)]
            )
    ...
    def test_extra_fields_forbidden(self):
        ...
        with pytest.raises(ValidationError):
            ResearchQueryValidator(query="Test", extra_field="should_fail")
...
class TestWebSocketMessageValidator:
    ...
    def test_session_id_validation(self):
        ...
        # Invalid format
        with pytest.raises(ValidationError):
            WebSocketMessageValidator(type="ping", session_id="not-a-uuid")
    ...
    def test_data_size_limit(self):
        ...
        with pytest.raises(ValueError, match="Message data too large"):
            WebSocketMessageValidator(type="research_query", data=large_data)
...
class TestSanitizationUtils:
    ...
    def test_json_output_sanitization(self):
        ...
        unsafe_data = {
            ...
            "items": [{"name": "<img src=x onerror=alert()>"}, {"name": "Safe item"}],
            "metadata": {
...
class TestValidationHelpers:
    ...
    def test_api_key_validation(self):
        ...
        # Valid API keys
        valid_keys = ["pk_" + "a" * 32, "pk_" + "0123456789abcdef" * 2]
...
class TestNexusForgeSecurity:
    ...
    def test_app_prompt_injection_protection(self, test_client, mock_user):
        ...
        with patch("src.api.routers.nexus_forge.get_current_user") as mock_auth:
            ...
            for malicious_prompt in injection_prompts:
                ...
                ), f"Server error with prompt: {malicious_prompt[:50]}..."
            ...
            with patch("src.api.routers.nexus_forge.orchestrator") as mock_orchestrator:
                ...
                if response.status_code == 200:
                    ...
                    for pattern in dangerous_patterns:
                        assert (
                            pattern not in response_text
                        ), f"Dangerous pattern not sanitized: {pattern}"
    ...
    def test_build_session_isolation(self, test_client):
        ...
        with patch("src.api.routers.nexus_forge.get_current_user") as mock_auth:
            ...
            response = test_client.get(
                f"/api/nexus-forge/build/{session2_id}", headers=auth_headers
            )

            assert (
                response.status_code == 403
            ), "User should not access other user's sessions"
    ...
    def test_websocket_security(self):
        ...
        with TestClient(app) as client:
            try:
                with client.websocket_connect(
                    f"/api/nexus-forge/ws/invalid-session"
                ) as websocket:
        ...
        try:
            with TestClient(app) as client:
                with client.websocket_connect(
                    f"/api/nexus-forge/ws/{session_id}"
                ) as websocket:
...
function App() {
    ...
    fetch(`/api/data?id=${userInput}`);  // No validation
        ...
        ), "Security scanning should detect vulnerabilities"
    ...
    def test_deployment_security(self):
        ...
        for url in deployment_urls:
            ...
            pattern = url.replace(url.split("-")[1], "SESSIONID").replace(
                url.split("-")[2].split(".")[0], "USERID"
            )
        ...
        assert (
            "SESSIONID" in list(url_patterns)[0]
        ), "URLs should include session identifier"
    ...
    def test_ai_model_rate_limiting(self, test_client, mock_user):
        ...
        with patch("src.api.routers.nexus_forge.get_current_user") as mock_auth:
            ...
            valid_responses = all(
                code
                in success_codes + rate_limit_codes + [400]  # 400 for validation errors
    ...
    def test_resource_exhaustion_protection(self, test_client, mock_user):
        ...
        with patch("src.api.routers.nexus_forge.get_current_user") as mock_auth:
            ...
            large_prompt = (
                "Build an app " + "with many features " * 1000
            )  # ~18KB prompt
            ...
            complex_config = {
                "nested": {"very": {"deeply": {"nested": {"config": True}}}}
            }
...
class TestNexusForgeAuthentication:
    ...
    def test_jwt_token_validation(self, test_client):
        ...
        for token in invalid_tokens:
            ...
            response = test_client.post(
                "/api/nexus-forge/build", json={"prompt": "test"}, headers=headers
            )
...
class TestNexusForgeCryptography:
    ...
    def test_session_id_generation(self):
        ...
        for session_id in session_ids[:10]:
            ...
            assert (
                session_id.count("-") == 4
            ), "Session ID should have proper UUID format"

    def test_api_key_security(self):
        """Test API key handling security"""
        # Test that API keys are not exposed in logs or responses
        api_keys = ["sk_test_123456789", "pk_live_987654321", "gemini_api_key_abcdefgh"]

        # Simulate configuration with API keys
        config = {"gemini_api_key": api_keys[2], "other_config": "safe_value"}
