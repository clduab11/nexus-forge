...
from nexus_forge.core.exceptions import SecurityError, AuthenticationError, AuthorizationError
...
class SecurityManager:
        ...
        self.default_roles = {
            "admin": UserRole(
                ...
                permissions={Permission.READ, Permission.WRITE, Permission.DELETE, 
                           Permission.EXECUTE, Permission.ADMIN, Permission.AGENT_CONTROL,
                           Permission.WORKFLOW_MANAGE, Permission.SYSTEM_CONFIG},
            ...
            "agent_operator": UserRole(
                ...
                permissions={Permission.READ, Permission.WRITE, Permission.EXECUTE,
                           Permission.AGENT_CONTROL, Permission.WORKFLOW_MANAGE},
            ...
            "developer": UserRole(
                ...
                resource_access={"agents": ["read"], "workflows": ["read", "create"], "tasks": ["read"]}
            ),
            "viewer": UserRole(
                role_id="viewer",
                name="Viewer",
                ...
                resource_access={"agents": ["read"], "workflows": ["read"], "tasks": ["read"]}
            )
        }
        ...
        try:
            ...
            if await self._is_account_locked(username):
                await self._log_audit_event(
                    None, None, "login_attempt", "user", username,
                    "blocked", "high", ip_address, user_agent,
            ...
            if not user_data:
                ...
                await self._log_audit_event(
                    None, None, "login_attempt", "user", username,
                    "failure", "medium", ip_address, user_agent,
            ...
            await self._log_audit_event(
                context.user_id, None, "login_success", "user", context.user_id,
                "success", "low", ip_address, user_agent, {}
            )
    ...
    async def authenticate_agent(
        ...
        try:
            ...
            if not agent_data:
                await self._log_audit_event(
                    None, agent_id, "agent_auth_attempt", "agent", agent_id,
                    "failure", "high", ip_address, None,
            ...
            context = SecurityContext(
                ...
                expires_at=datetime.now(timezone.utc) + timedelta(hours=24)  # Longer for agents
            )
            
            self.active_sessions[context.session_id] = context
            
            await self._log_audit_event(
                None, agent_id, "agent_auth_success", "agent", agent_id,
                "success", "low", ip_address, None, {}
            )
    ...
    async def authorize_action(
        ...
        try:
            ...
            if required_permission not in context.permissions:
                await self._log_audit_event(
                    context.user_id, context.agent_id, action, resource_type, resource_id,
                    "blocked", "medium", context.ip_address, None,
                    {"reason": "insufficient_permissions", "required": required_permission.value}
                )
                return False
            ...
            if context.security_level.value < required_level.value:
                await self._log_audit_event(
                    context.user_id, context.agent_id, action, resource_type, resource_id,
                    "blocked", "high", context.ip_address, None,
            ...
            if not await self._check_rate_limit(context, action):
                await self._log_audit_event(
                    context.user_id, context.agent_id, action, resource_type, resource_id,
                    "blocked", "low", context.ip_address, None,
            ...
            await self._log_audit_event(
                context.user_id, context.agent_id, action, resource_type, resource_id,
                "success", "low", context.ip_address, None, {}
            )
    ...
    async def validate_input(self, input_data: str, input_type: str = "general") -> bool:
        ...
        try:
            ...
            if await self._detect_prompt_injection(input_data):
                await self._log_audit_event(
                    None, None, "prompt_injection_detected", "input", None,
                    "blocked", "critical", None, None,
            ...
            if await self._detect_malicious_patterns(input_data):
                await self._log_audit_event(
                    None, None, "malicious_input_detected", "input", None,
                    "blocked", "high", None, None,
    ...
    async def invalidate_session(self, session_id: str):
        ...
        if session_id in self.active_sessions:
            ...
            await self._log_audit_event(
                context.user_id, context.agent_id, "session_invalidated", "session", session_id,
                "success", "low", context.ip_address, None, {}
            )
    ...
    async def export_compliance_report(
        ...
        audit_events = await self.get_audit_trail(start_time=start_date, end_time=end_date)
        ...
        report = {
            ...
            "summary": {
                ...
                "successful_actions": len([e for e in audit_events if e.result == "success"]),
                "failed_actions": len([e for e in audit_events if e.result == "failure"]),
                "blocked_actions": len([e for e in audit_events if e.result == "blocked"]),
                "high_risk_events": len([e for e in audit_events if e.risk_level in ["high", "critical"]])
            },
            "events": [asdict(event) for event in audit_events],
            "security_policies": [asdict(policy) for policy in self.security_policies.values()],
    ...
    async def _is_account_locked(self, username: str) -> bool:
        ...
        recent_attempts = [
            ...
            if attempt > datetime.now(timezone.utc) - timedelta(minutes=self.lockout_duration_minutes)
        ]
    ...
    async def _validate_agent_credentials(self, agent_id: str, api_key: str) -> Optional[Dict[str, Any]]:
        """Validate agent credentials (mock implementation)"""
        # Mock agent database
        mock_agents = {
            "starri-orchestrator": {"api_key": "starri-key-123", "security_level": "secret"},
            "gemini-analysis": {"api_key": "gemini-key-456", "security_level": "secret"},
    ...
    async def _is_session_valid(self, context: SecurityContext) -> bool:
        ...
        return (
            context.session_id in self.active_sessions and
            datetime.now(timezone.utc) < context.expires_at
        )
    ...
    def _get_required_security_level(self, resource_type: str, action: str) -> SecurityLevel:
    ...
    async def _check_resource_access(
        ...
        for role in context.roles:
            if resource_type in role.resource_access:
                ...
                if "*" in allowed_resources or (resource_id and resource_id in allowed_resources):
                    return True
    ...
    async def _apply_output_filtering(self, text: str, context: SecurityContext) -> str:
        ...
        if context.security_level == SecurityLevel.PUBLIC:
            ...
            text = re.sub(r'\b(internal|confidential|secret)\b', '[FILTERED]', text, flags=re.IGNORECASE)
    ...
    async def _log_audit_event(
        ...
        if risk_level in ["high", "critical"]:
            logger.warning(f"High-risk security event: {action} by {user_id or agent_id} - {result}")
