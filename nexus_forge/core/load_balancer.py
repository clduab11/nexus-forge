...
class ServerNode:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            self.capacity_utilization < 0.9 and
            (datetime.utcnow() - self.last_health_check).seconds < 30
        )
...
class AdvancedLoadBalancer:
    ...
    async def select_server(self, request_context: Optional[Dict[str, Any]] = None) -> Optional[ServerNode]:
        """Select the best server based on the current strategy"""
        # Filter healthy servers
        healthy_servers = [node for node in self.server_nodes.values() if node.is_healthy]
    ...
    def _weighted_round_robin_selection(self, servers: List[ServerNode]) -> ServerNode:
        ...
        selected_server = weighted_servers[self._round_robin_index % len(weighted_servers)]
        self._round_robin_index += 1
        return selected_server
    ...
    async def _predictive_selection(self, servers: List[ServerNode], request_context: Optional[Dict[str, Any]]) -> ServerNode:
    ...
    async def _adaptive_selection(self, servers: List[ServerNode], request_context: Optional[Dict[str, Any]]) -> ServerNode:
    ...
    async def _predict_server_loads(self, servers: List[ServerNode]) -> Dict[str, float]:
        ...
        for server in servers:
            ...
            if len(recent_metrics) >= 2:
                ...
                load_trend = (recent_metrics[-1].requests_per_second - recent_metrics[0].requests_per_second) / len(recent_metrics)
    ...
    async def handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                response = await self._process_request_on_server(selected_server, request)
    ...
    async def _process_request_on_server(self, server: ServerNode, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _update_server_metrics(self, server: ServerNode, response_time: float, success: bool):
        """Update server performance metrics"""
        # Update average response time with exponential moving average
        alpha = 0.1  # Smoothing factor
        server.avg_response_time = (1 - alpha) * server.avg_response_time + alpha * response_time
        
        # Update capacity utilization
        connection_ratio = server.current_connections / server.max_connections
        server.capacity_utilization = max(connection_ratio, server.capacity_utilization * 0.95)  # Decay factor
    ...
    def _record_request(self, request: Dict[str, Any], server: ServerNode, response_time: float, success: bool):
    ...
    async def _check_auto_scaling(self):
        ...
        total_load = sum(server.capacity_utilization for server in self.server_nodes.values())
        ...
        elif avg_load < self.scale_down_threshold and current_server_count > self.min_nodes:
    ...
    async def _scale_down(self):
        ...
        least_utilized = min(self.server_nodes.values(), key=lambda s: s.capacity_utilization)
    ...
    def get_load_balancing_stats(self) -> Dict[str, Any]:
        ...
        total_connections = sum(server.current_connections for server in self.server_nodes.values())
        total_capacity = sum(server.max_connections for server in self.server_nodes.values())
        
        avg_health = statistics.mean([server.health_score for server in self.server_nodes.values()]) if self.server_nodes else 0
        avg_response_time = statistics.mean([server.avg_response_time for server in self.server_nodes.values()]) if self.server_nodes else 0
        ...
        return {
            ...
            "healthy_servers": sum(1 for server in self.server_nodes.values() if server.is_healthy),
            "total_connections": total_connections,
            "total_capacity": total_capacity,
            "capacity_utilization": total_connections / total_capacity if total_capacity > 0 else 0,
...
async def setup_demo_load_balancer() -> AdvancedLoadBalancer:
    ...
    servers = [
        ...
        ServerNode("server_4", "10.0.0.13", 8004, weight=1, max_connections=400, region="us-east"),
        ServerNode("server_5", "10.0.0.14", 8005, weight=2, max_connections=600, region="us-west")
    ]
    ...
    async def demo_load_balancer():
        ...
        for i in range(50):
            ...
            if i % 10 == 0:
                ...
                logger.info(f"Stats: {stats['healthy_servers']}/{stats['server_count']} healthy, "
