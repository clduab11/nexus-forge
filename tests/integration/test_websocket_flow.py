...
class TestWebSocketFlow:
    ...
    async def test_websocket_connection_lifecycle(
        ...
        sent_messages = [e["data"] for e in connection_events if e["type"] == "message_sent"]
        assert len(sent_messages) >= len(phases) + 2  # phases + handshake + completion
        
        # Verify progress updates
        progress_updates = [m for m in sent_messages if m.get("type") == "progress_update"]
    ...
    async def test_concurrent_websocket_connections(
        ...
        for timing in connection_times:
            assert timing["duration"] < performance_benchmarks["websocket_connection_time"]
                ...
                try:
                    ...
                    if scenario["recovery"] == "reconnect":
                        ...
                        if attempt == scenario["max_retries"] - 1:
                            ...
                            mock_nexus_forge_websocket.receive_json.return_value = {"type": "ping"}
    ...
    async def test_websocket_message_ordering_and_buffering(
        ...
        for i in range(1, len(received_messages)):
            ...
            assert curr_seq > prev_seq, f"Message order violated: {prev_seq} -> {curr_seq}"
        
        # Timestamps should be monotonically increasing
        timestamps = [datetime.fromisoformat(msg["timestamp"]) for msg in received_messages]
    ...
    async def test_websocket_performance_metrics(
        ...
        for scenario in test_scenarios:
            ...
            assert throughput >= scenario["expected_throughput"] * 0.8  # Allow 20% variance
