...
def generate_cache_key(func_name: str, args: tuple, kwargs: dict, 
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.debug(f"Cached result for {func.__name__} with TTL {actual_ttl}s")
                return result
...
def semantic_cache(
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            similar_result = _find_similar_cached_result(cache, func.__name__, prompt, similarity_threshold)
            if similar_result:
            ...
            cache.set(cache_key, {
                ...
                'prompt': prompt,
                'timestamp': time.time()
            }, actual_ttl, CacheStrategy.SEMANTIC)
...
def _find_similar_cached_result(cache: RedisCache, func_name: str, prompt: str, 
    ...
    try:
        ...
        for key in keys:
            try:
                cached_data = cache.get(key.decode() if isinstance(key, bytes) else key, CacheStrategy.SEMANTIC)
                ...
                if similarity >= threshold:
                    logger.debug(f"Found similar cached result with similarity {similarity:.2f}")
...
def _generate_cache_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if hit_rate < 50:
        recommendations.append("Consider increasing TTL values or improving cache key generation")
    elif hit_rate > 90:
        recommendations.append("Excellent cache performance! Consider expanding caching to more functions")
    
    evictions = stats.get('evictions', 0)
    if evictions > 100:
        recommendations.append("High eviction rate detected. Consider increasing cache size or optimizing data storage")
