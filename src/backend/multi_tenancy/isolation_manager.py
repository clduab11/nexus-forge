...
from .models import IsolationLevel, Tenant, TenantResource, TenantTier
...
class TenantIsolationManager:
    ...
    async def create_tenant_isolation(self, tenant: Tenant) -> List[TenantResource]:
        ...
        logger.info(
            f"Creating isolation for tenant {tenant.slug} (tier: {tenant.tier})"
        )
        ...
        try:
            ...
            logger.info(
                f"Created {len(resources)} isolation resources for tenant {tenant.slug}"
            )
    ...
    async def update_tenant_isolation(
        ...
        logger.info(
            f"Updating isolation for tenant {tenant.slug} from {tenant.tier} to {new_tier}"
        )
        ...
        if not old_config.get("namespace_isolation") and new_config.get(
            "namespace_isolation"
        ):
        ...
        if not old_config.get("dedicated_database") and new_config.get(
            "dedicated_database"
        ):
        ...
        if not old_config.get("dedicated_cluster") and new_config.get(
            "dedicated_cluster"
        ):
    ...
    async def _create_kubernetes_namespace(self, tenant: Tenant) -> TenantResource:
        ...
        return TenantResource(
            ...
            provider="kubernetes",
            region="local",
            config={"namespace": namespace_name, "manifest": namespace_manifest},
        )
    ...
    async def _apply_resource_quotas(self, tenant: Tenant) -> TenantResource:
        ...
        quota_manifest = {
            "apiVersion": "v1",
            "kind": "ResourceQuota",
            "metadata": {"name": quota_name, "namespace": namespace_name},
            "spec": {"hard": limits},
        }
                ...
                "ingress": [
                    {
                        "from": [
                            {
                                "namespaceSelector": {
                                    "matchLabels": {"name": namespace_name}
                                }
                            },
                            {
                                "namespaceSelector": {
                                    "matchLabels": {"name": "nexus-forge-system"}
                                }
                            },
                        ]
                    }
    ...
    async def _create_database_isolation(self, tenant: Tenant) -> TenantResource:
        ...
        try:
            ...
            await self.supabase.client.rpc(
                "setup_tenant_rls", {"schema_name": schema_name, "tenant_id": tenant.id}
            ).execute()
            ...
            return TenantResource(
                ...
                provider="supabase",
                region="global",
                config={"schema_name": schema_name, "rls_enabled": True},
            )
    ...
    async def _delete_database_isolation(self, tenant: Tenant) -> bool:
        ...
        try:
            await self.supabase.client.rpc(
                "drop_tenant_schema", {"schema_name": schema_name}
            ).execute()
    ...
    async def _delete_tenant_resources(self, tenant_id: str) -> bool:
        ...
        try:
            await self.supabase.client.table("tenant_resources").delete().eq(
                "tenant_id", tenant_id
            ).execute()
            return True
        except Exception as e:
        ...
        for resource in resources:
            try:
                if resource.resource_type == "kubernetes_namespace":
                    await self._delete_kubernetes_namespace_by_name(
    ...
    async def _validate_database_isolation(self, tenant: Tenant) -> bool:
        ...
        try:
            ...
            result = await self.supabase.client.rpc(
                "validate_tenant_isolation", {"tenant_id": tenant.id}
            ).execute()
