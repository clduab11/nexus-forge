...
"""
        ...
        assert any("injection" in issue["description"].lower() 
...
class TestAgentSelfImprovementOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_improvement_recommendation(self, improvement_orchestrator, sample_agent_performance):
        """Test improvement recommendation generation"""
        recommendations = await improvement_orchestrator.generate_improvement_recommendations(
    ...
    async def test_improvement_execution(self, improvement_orchestrator):
        ...
        with patch.object(improvement_orchestrator.rl_engine, 'optimize_agent_policy') as mock_rl:
            mock_rl.return_value = {"optimization_applied": True, "improvement": 0.05}
            
            result = await improvement_orchestrator.execute_improvement(
                "test_agent", ImprovementStrategy.REINFORCEMENT_LEARNING, {"learning_rate": 0.1}
            )
    ...
    async def test_complete_improvement_cycle(self, improvement_orchestrator, sample_agent_performance):
        """Test complete improvement cycle"""
        # Mock external dependencies
        with patch.object(improvement_orchestrator, '_analyze_agent_performance') as mock_analyze:
...
class TestGlobalFunctions:
    ...
    async def test_start_improvement_system(self):
        ...
        with patch('nexus_forge.features.agent_self_improvement.get_self_improvement_orchestrator') as mock_get:
...
class TestErrorHandling:
    ...
    async def test_network_timeout_handling(self, improvement_orchestrator):
        ...
        with patch.object(improvement_orchestrator.debate_system, 'conduct_debate_round') as mock_debate:
...
class TestSystemIntegration:
    ...
    async def test_mem0_knowledge_integration(self, improvement_orchestrator):
        ...
        with patch.object(improvement_orchestrator, '_store_learning_patterns') as mock_store:
        ...
        with patch.object(improvement_orchestrator.cache, 'get_l2') as mock_get:
            mock_get.return_value = {
                "agent_behavior_patterns": {
                    "test_agent": {"collaboration_score": 0.9, "efficiency_trends": [0.8, 0.85, 0.9]}
                }
            }
