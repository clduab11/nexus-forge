...
async def test_all_systems_parallel():
    ...
    for i, result in enumerate(results):
        test_name = (
            health_tests[i].__name__
            if hasattr(health_tests[i], "__name__")
            else f"test_{i}"
        )
    ...
    logging.info(
        f"📊 System Health Summary: {passed_tests}/{passed_tests + failed_tests} tests passed ({success_rate:.1f}%)"
    )

    # Require at least 85% success rate
    assert (
        success_rate >= 85
    ), f"System health below threshold: {success_rate:.1f}% (minimum 85%)"
