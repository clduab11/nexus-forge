...
class TestADKIntegration:
        ...
        async def mock_stream_query(query):
            ...
            events = [
                ...
                Mock(
                    type="delegating",
                    agent_name="orchestrator",
                    content="Delegating to retrieval",
                    progress=10,
                    metadata={},
                ),
                Mock(
    ...
    async def test_stream_research_basic(self, adk_client, mock_streaming_session):
        ...
        with patch(
            "src.api.adk_integration.StreamingSession",
            return_value=mock_streaming_session,
        ):
            events = []

            async for event in adk_client.stream_research(
                "What is quantum computing?", "user123", "session456", "comprehensive"
            ):
                events.append(event)
    ...
    async def test_stream_research_error_handling(self, adk_client):
        ...
        with patch(
            "src.api.adk_integration.StreamingSession", return_value=error_session
        ):
            events = []
    ...
    async def test_agent_health_check_failure(self, adk_client):
        ...
        adk_client.agents["retrieval"].aquery = AsyncMock(
    ...
    async def test_multi_agent_coordination(self, adk_client):
        ...
        with patch(
            "src.api.adk_integration.StreamingSession",
            return_value=coordination_session,
        ):
            events = []

            async for event in adk_client.stream_research(
                "Complex research query", "user123", "session456"
            ):
                events.append(event)
                    ...
                    for i in range(10):
                        task = asyncio.create_task(
                            self._collect_events(
                                client, f"Query {i}", f"user{i}", f"session{i}"
                            )
                        )
                        tasks.append(task)
