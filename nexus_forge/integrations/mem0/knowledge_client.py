...
class Mem0KnowledgeClient:
    ...
    def __init__(
        ...
        logger.info(f"Initialized Mem0 knowledge client for orchestrator: {orchestrator_id}")
    ...
    async def create_entity(self, entity_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(f"Created entity: {entity_id} of type {entity_data.get('type')}")
            return entity_id
    ...
    async def create_relationship(
        ...
        try:
            ...
            relationship = {
                ...
                "from": from_entity,
                "to": to_entity,
                "type": self.relationship_types.get(relationship_type, relationship_type),
    ...
    async def get_agent_relationships(
        ...
        if relationship_type:
            relationships = [
                ...
                if rel["type"] == self.relationship_types.get(relationship_type, relationship_type)
            ]
    ...
    async def get_agent_performance_metrics(
        ...
        if agent_entity:
            return agent_entity.get("properties", {}).get("performance_metrics", {
                "tasks_completed": 0,
                "success_rate": 0.0,
                "average_execution_time": 0.0
            })
    ...
    async def save_orchestrator_state(
        ...
        try:
            ...
            await self.update_entity_metrics(orchestrator_id, {
                "metrics": metrics,
            ...
            logger.info(f"Saved orchestrator state with {len(thinking_chains)} thinking chains")
    ...
    async def export_knowledge_graph(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "relationships": [
                rel 
                for rels in self.relationship_cache.values() 
                for rel in rels
            ],
    ...
    async def import_knowledge_graph(self, graph_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Imported knowledge graph with {len(self.entity_cache)} entities")
