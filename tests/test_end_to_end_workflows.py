...
class TestCompleteWorkflows:
    ...
    async def full_orchestrator(self):
        ...
        with patch('nexus_forge.agents.starri.orchestrator.GeminiClient'):
            with patch('nexus_forge.agents.starri.orchestrator.SupabaseCoordinationClient'):
                with patch('nexus_forge.agents.starri.orchestrator.Mem0KnowledgeClient'):
                    ...
                    orchestrator.coordination_client.register_agent.return_value = "orch_agent_123"
                    orchestrator.knowledge_client.initialize_orchestrator_knowledge.return_value = None
        ...
        agents = [
            {
                "id": "ui_designer_001",
                "type": "ui_designer",
                "capabilities": [AgentCapability.UI_DESIGN, AgentCapability.IMAGE_GENERATION]
            },
            {
                "id": "backend_dev_001", 
                "type": "backend_developer",
                "capabilities": [AgentCapability.CODE_GENERATION, AgentCapability.API_INTEGRATION]
            },
            {
            ...
            },
            {
                "id": "qa_engineer_001",
                "type": "qa_engineer",
                "capabilities": [AgentCapability.TESTING, AgentCapability.DATA_ANALYSIS]
            }
        ]
        ...
        full_orchestrator.coordinate_agents = AsyncMock(
            return_value={
                ...
                "results": {
                    ...
                    "implement_frontend": {
                        "files": ["src/App.js", "src/components/TaskCard.js", "src/pages/Dashboard.js"],
                ...
                "reflection": {
                    ...
                    "quality_score": 0.95,
                    "improvements": ["Consider implementing caching for better performance"]
                },
                "metrics": {
    ...
    async def test_complex_data_dashboard_workflow(self, full_orchestrator):
        ...
        full_orchestrator.think_deeply = AsyncMock(
            side_effect=[
                # Initial analysis
                {
                    ...
                    "conclusion": {"conclusion": "Complex analytics dashboard with microservices", "confidence": 0.9},
                    "confidence": 0.9,
                    "mode": "deep_analysis"
                },
                # Coordination planning
                {
                    "conclusion": {
                        ...
                        "critical_path": ["data_pipeline", "api_optimization", "dashboard_ui"]
                    },
                    "confidence": 0.88,
                    "mode": "coordination"
                },
                # Execution reflection
                {
                    ...
                    "quality_score": 0.92,
                    "improvements": ["Add monitoring and alerting", "Implement caching strategy"],
                    "mode": "reflection"
                }
            ]
        )
        ...
        full_orchestrator._parse_task_decomposition = MagicMock(
            return_value={
                ...
                "critical_path": ["design_data_architecture", "build_data_pipeline", "optimize_performance"]
            }
        )
        
        # Mock workflow creation and validation
        full_orchestrator.coordination_client.create_workflow.return_value = "complex_dashboard_workflow_001"
        full_orchestrator._validate_decomposition = AsyncMock(
            return_value={"valid": True, "errors": [], "warnings": ["High complexity project"]}
        )
            ...
            return_value={
                ...
                "results": {
                    "design_data_architecture": {
                        ...
                        "performance_requirements": {"throughput": "10k rps", "latency": "<100ms"}
                    },
                    "build_data_pipeline": {
                        "pipeline_code": ["ingestion.py", "processing.py", "streaming.py"],
        ...
        decomposition = await full_orchestrator.decompose_task(
            ...
            constraints={
                "performance": {"max_response_time": "100ms", "min_throughput": "10k rps"},
        ...
        throughput = int(results["build_data_pipeline"]["throughput_achieved"].split("k")[0]) * 1000
        assert throughput >= 10000  # Met throughput requirement
        
        response_time = int(results["optimize_performance"]["response_time"].split()[1].rstrip("ms"))
        assert response_time <= 100  # Met latency requirement
        
        # Verify quality standards
        test_coverage = float(results["comprehensive_testing"]["test_coverage"].rstrip("%"))
    ...
    async def test_workflow_error_recovery(self, full_orchestrator):
        ...
        full_orchestrator.think_deeply = AsyncMock(
            return_value={
                "conclusion": {"conclusion": "Standard web app workflow", "confidence": 0.8},
                "confidence": 0.8
            }
        )
        ...
        full_orchestrator.coordination_client.create_task = AsyncMock(side_effect=["t1", "t2", "t3"])
        ...
        full_orchestrator._reflect_on_execution = AsyncMock(
            return_value={
                ...
                "lessons_learned": ["Implement better error handling", "Add retry mechanisms"]
            }
        )
    ...
    async def test_concurrent_workflows(self, full_orchestrator):
        ...
        full_orchestrator.think_deeply = AsyncMock(
            return_value={
                "conclusion": {"conclusion": "Concurrent workflow execution", "confidence": 0.85},
                "confidence": 0.85
            }
        )
        ...
        async def mock_decompose_task(task_description, requirements, constraints=None):
            ...
            return {
                ...
                "decomposition": {
                    "subtasks": [
                        {"id": f"task_{workflow_counter}_1", "description": f"Task 1 for workflow {workflow_counter}"},
                        {"id": f"task_{workflow_counter}_2", "description": f"Task 2 for workflow {workflow_counter}"}
                    ]
                }
            }
        ...
        for i, result in enumerate(results):
            assert result["decomposition"]["workflow_id"] == f"concurrent_workflow_{i + 1}"
...
class TestPerformanceBenchmarks:
    ...
    async def test_thinking_performance_benchmark(self, benchmark_orchestrator):
        ...
        assert total_time < 5.0  # Should complete 10 thinking processes in under 5 seconds
        assert all(result["confidence"] > 0.8 for result in results)
        
        # Calculate average thinking time
        avg_thinking_time = sum(result["thinking_time"] for result in results) / len(results)
    ...
    async def test_agent_registration_scalability(self, benchmark_orchestrator):
        ...
        benchmark_orchestrator.knowledge_client.add_agent_entity = AsyncMock(return_value="entity_id")
        ...
        assert len(benchmark_orchestrator.agent_capabilities[AgentCapability.CODE_GENERATION]) == 100
