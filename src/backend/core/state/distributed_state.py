...
class DistributedStateManager:
    ...
    async def update_session_state(
        self, session_id: str, updates: Dict[str, Any], ttl: Optional[int] = None
    ):
        """
    ...
    async def create_research_task(
        self, user_id: str, query: str, mode: str = "comprehensive"
    ) -> str:
        """
        ...
        # Store in Firestore
        await self.firestore.collection("research_tasks").document(task_id).set(
            task_data
        )
    ...
    async def update_task_progress(
        ...
        updates = {"progress": progress, "updated_at": datetime.now().isoformat()}
        ...
        try:
            ...
            if task_json:
                ...
                await redis_client.setex(
                    f"task:{task_id}", 86400, json.dumps(task_data)
                )
        except redis.RedisError:
            pass

        # Update Firestore
        await self.firestore.collection("research_tasks").document(task_id).update(
            updates
        )
    ...
    async def distributed_lock(
        self, resource: str, timeout: int = 10, blocking: bool = True
    ):
        """
    ...
    async def set_cached(self, key: str, value: Any, ttl: Optional[int] = None):
        ...
        try:
            await redis_client.setex(
                f"cache:{key}", ttl or self.cache_ttl, json.dumps(value)
            )
