...
class InteractionLogger:
    ...
    async def log_interaction(self, interaction: AgentInteraction):
        ...
        try:
            ...
            self.log_stats["buffer_utilization"] = (
                len(self.interaction_buffer) / self.interaction_buffer.maxlen
            )
    ...
    async def _buffer_flush_worker(self):
        ...
        while self.logging_active:
            try:
                if (
                    len(self.interaction_buffer) >= 100
    ...
    async def _flush_buffer(self):
        ...
        try:
            ...
            batch_data = {
                ...
                "timestamp": time.time(),
                "interactions": [
                    asdict(interaction) for interaction in interactions_to_flush
                ],
            }

            await self.cache.set_l3(
                f"interaction_batch:{batch_id}", batch_data, timeout=604800
            )  # 1 week
        ...
        while self.logging_active:
            try:
                ...
                if interactions_per_second > 0:
                    logger.debug(
                        f"Logging performance: {interactions_per_second:.2f} interactions/sec"
                    )
    ...
    async def get_interactions(
        ...
        for interaction in self.interaction_buffer:
            if self._matches_filters(
                interaction, agent_id, time_range, interaction_types
            ):
        ...
        if len(interactions) < limit:
            ...
            for key in batch_keys[:10]:  # Limit search to recent batches
                try:
                    batch_data = await self.cache.get(
    ...
    def _matches_filters(
        ...
        if (
            agent_id
            and interaction.source_agent != agent_id
            and interaction.target_agent != agent_id
        ):
            return False
...
class PatternAnalyzer:
    ...
    async def analyze_behavior_patterns(
        ...
        model: AnalysisModel = AnalysisModel.ENSEMBLE,
    ) -> BehaviorAnalysis:
        """
        ...
        agents_analyzed = list(
            set(
                [i.source_agent for i in interactions]
                + [i.target_agent for i in interactions if i.target_agent]
            )
        )

        logger.info(
            f"Analyzing behavior patterns for {len(agents_analyzed)} agents using {model.value}"
        )

        # Detect individual agent patterns
        patterns_detected = await self._detect_individual_patterns(
            interactions, agents_analyzed
        )
        ...
        performance_insights = await self._extract_performance_insights(
            interactions, agents_analyzed
        )
        ...
        anomalies_detected = await self._detect_anomalies(
            interactions, patterns_detected
        )
        ...
        await self.cache.set_l2(
            f"behavior_analysis:{analysis_id}", asdict(analysis), timeout=86400
        )
        ...
        logger.info(
            f"Behavior analysis completed: {len(patterns_detected)} patterns, "
            f"{len(optimization_recommendations)} recommendations"
        )

        return analysis

    async def _detect_individual_patterns(
        self, interactions: List[AgentInteraction], agents: List[str]
        ...
        for agent_id in agents:
            ...
            if (
                len(agent_interactions) < 10
            ):  # Minimum interactions for pattern detection
                continue

            # Analyze different pattern types
            agent_patterns = await self._analyze_agent_patterns(
                agent_id, agent_interactions
            )
    ...
    async def _analyze_agent_patterns(
        self, agent_id: str, interactions: List[AgentInteraction]
    ) -> List[BehaviorPattern]:
        ...
        if (
            metrics["collaboration_rate"] > 0.7
            and metrics["success_rate"] > 0.9
            and metrics["average_response_time"] < 1000
        ):
            ...
            pattern = BehaviorPattern(
                ...
                confidence=min(
                    metrics["collaboration_rate"] + metrics["success_rate"], 1.0
                )
                / 2,
                ...
                impact_score=0.8,
                recommendations=[
                    "Leverage as mentor for other agents",
        ...
        if metrics["average_response_time"] > 5000 and metrics["error_rate"] > 0.2:
            ...
            pattern = BehaviorPattern(
                ...
                confidence=min(
                    metrics["error_rate"] + (metrics["average_response_time"] / 10000),
                    1.0,
                ),
                frequency=len(interactions),
                first_detected=min(i.timestamp for i in interactions),
        ...
        if metrics["average_response_time"] < 500 and metrics["success_rate"] > 0.85:

            pattern = BehaviorPattern(
                pattern_id=f"{agent_id}_rapid_responder_{int(time.time())}",
                agent_id=agent_id,
                pattern_type=AgentBehaviorPattern.RAPID_RESPONDER,
                confidence=(1.0 - metrics["average_response_time"] / 1000)
    ...
    async def _calculate_agent_metrics(
        ...
        collaboration_interactions = sum(
            1
            for i in interactions
            if i.interaction_type == InteractionType.COLLABORATION
        )
        collaboration_rate = (
            collaboration_interactions / total_interactions
            if total_interactions > 0
            else 0
        )
        ...
        # Error analysis
        error_types = [
            i.error_details.get("type", "unknown")
            for i in interactions
    ...
    async def _calculate_learning_trend(
        ...
        early_success_rate = sum(1 for i in early_interactions if i.success) / len(
            early_interactions
        )
        recent_success_rate = sum(1 for i in recent_interactions if i.success) / len(
            recent_interactions
        )

        # Calculate response time improvement
        early_response_times = [
            i.duration for i in early_interactions if i.duration is not None
        ]
        recent_response_times = [
            i.duration for i in recent_interactions if i.duration is not None
        ]

        early_avg_response = (
            np.mean(early_response_times) if early_response_times else 0
        )
        recent_avg_response = (
            np.mean(recent_response_times) if recent_response_times else 0
        )
        ...
        if early_avg_response > 0 and recent_avg_response > 0:
            response_improvement = (
                early_avg_response - recent_avg_response
            ) / early_avg_response
        else:
    ...
    async def _build_collaboration_graph(
        ...
        nodes = {
            agent: {"interaction_count": 0, "success_rate": 0.0} for agent in agents
        }
        edges = {}
        ...
        for interaction in interactions:
            ...
            if interaction.success:
                nodes[source]["success_rate"] = (
                    nodes[source].get("success_rate", 0)
                    * (nodes[source]["interaction_count"] - 1)
                    + 1
        ...
        for edge_key, edge_data in edges.items():
            if edge_data["interaction_count"] > 0:
                success_rate = (
                    edge_data["success_count"] / edge_data["interaction_count"]
                )
                avg_duration = (
                    edge_data["total_duration"] / edge_data["interaction_count"]
                )

                # Collaboration strength based on frequency, success rate, and efficiency
                collaboration_strength = (
                    min(edge_data["interaction_count"] / 10, 1.0)
                    * 0.4  # Frequency factor
    ...
    async def _calculate_centrality(
        self, agents: List[str], edges: Dict[Tuple[str, str], Dict[str, Any]]
    ) -> Dict[str, float]:
        ...
        if max_centrality > 0:
            centrality = {
                agent: score / max_centrality for agent, score in centrality.items()
            }
    ...
    async def _detect_communities(
        ...
        communities["moderate_collaboration"] = [
            agent for agent in agents if agent not in strong_collaborators
        ]
    ...
    async def _calculate_efficiency_metrics(
        ...
        total_strength = sum(
            edge_data.get("collaboration_strength", 0) for edge_data in edges.values()
        )
        ...
        network_density = (
            len(edges) / total_possible_edges if total_possible_edges > 0 else 0
        )
    ...
    async def _extract_performance_insights(
        ...
        for agent in agents:
            ...
            if agent_interactions:
                ...
                agent_performance[agent] = (
                    ...
                    + (1 - min(metrics.get("average_response_time", 1000) / 1000, 1))
                    * 0.4
                )

        insights["agent_rankings"] = dict(
            sorted(agent_performance.items(), key=lambda x: x[1], reverse=True)
        )
        ...
        for agent in agents:
            ...
            if agent_interactions:
                metrics = await self._calculate_agent_metrics(agent, agent_interactions)
                if (
                    metrics.get("average_response_time", 0) > 3000
                    or metrics.get("error_rate", 0) > 0.2
                ):
                    bottlenecks.append(
                        {
                            "agent": agent,
                            ...
                            "issues": [
                                (
                                    "High response time"
                                    if metrics.get("average_response_time", 0) > 3000
                                    else ""
                                ),
                                (
                                    "High error rate"
    ...
    async def _generate_optimization_recommendations(
        ...
        bottlenecks = performance_insights.get("bottleneck_analysis", {}).get(
            "bottleneck_agents", []
        )
        ...
        overall_success_rate = performance_insights.get("overall_metrics", {}).get(
            "success_rate", 0
        )
    ...
    async def _detect_anomalies(
        self, interactions: List[AgentInteraction], patterns: List[BehaviorPattern]
    ) -> List[Dict[str, Any]]:
        ...
        if len(interactions) >= 50:
            ...
            earlier_success_rate = sum(
                1 for i in earlier_interactions if i.success
            ) / len(earlier_interactions)
                ...
                anomalies.append(
                    {
                        ...
                        "metrics": {
                            ...
                            "drop_percentage": (
                                earlier_success_rate - recent_success_rate
                            )
                            * 100,
                        },
                        "timestamp": time.time(),
        ...
        if (
            len(collaboration_interactions) < len(interactions) * 0.1
        ):  # Less than 10% collaboration
            anomalies.append(
                {
                    ...
                    "metrics": {
                        "collaboration_rate": len(collaboration_interactions)
                        / len(interactions)
                        * 100,
                    ...
                    anomalies.append(
                        {
                            ...
                            "metrics": {
                                ...
                                / len(
                                    [i for i in interactions if i.source_agent == agent]
                                ),
                            },
                            "timestamp": time.time(),
                        }
    ...
    async def _calculate_prediction_confidence(
        self, patterns: List[BehaviorPattern], collaboration_graph: CollaborationGraph
    ) -> float:
        ...
        connected_agents = len(
            [
                agent
                for agent, centrality in collaboration_graph.centrality_scores.items()
                if centrality > 0
            ]
        )
        ...
        data_sufficiency = min(
            total_patterns / (total_agents * 2), 1.0
        )  # Want ~2 patterns per agent
...
class BehaviorAnalysisOrchestrator:
    ...
    async def _run_periodic_analysis(self):
        ...
        try:
            ...
            interactions = await self.logger.get_interactions(
                time_range=(cutoff_time, time.time()), limit=1000
            )
            ...
            self.system_metrics["anomalies_detected"] += len(
                analysis.anomalies_detected
            )
            self.system_metrics["optimization_recommendations"] += len(
            ...
            logger.info(
                f"Analysis completed: {len(analysis.patterns_detected)} patterns, "
    ...
    async def _handle_critical_findings(self, analysis: BehaviorAnalysis):
        ...
        for pattern in analysis.patterns_detected:
            if pattern.pattern_type in [
                AgentBehaviorPattern.BOTTLENECK_CREATOR,
                AgentBehaviorPattern.ERROR_PRONE,
            ]:
                if pattern.confidence > 0.8:
                    critical_issues.append(
                        f"Critical pattern: {pattern.pattern_type.value} "
        ...
        critical_recommendations = [
            r
            for r in analysis.optimization_recommendations
        ...
        if critical_recommendations:
            critical_issues.extend(
                [
                    f"Critical recommendation: {r['title']}"
    ...
    async def _quick_anomaly_check(self, interactions: List[AgentInteraction]):
        ...
        if error_rate > 0.5:  # More than 50% errors
            logger.warning(
                f"High error rate detected: {error_rate:.1%} in recent interactions"
            )
        ...
        if response_times:
            ...
            if avg_response_time > 10000:  # More than 10 seconds
                logger.warning(
                    f"High response time detected: {avg_response_time:.1f}ms average"
                )
        ...
        while self.monitoring_active:
            try:
                ...
                self.system_metrics.update(
                    {
                        "interactions_per_second": logging_stats.get(
                            "interactions_per_second", 0
                        ),
                        "buffer_utilization": logging_stats.get(
    ...
    async def analyze_agent_behavior(
        self, agent_id: str, time_range: Optional[Tuple[float, float]] = None
    ) -> Optional[BehaviorAnalysis]:
        """Analyze behavior for a specific agent"""
        interactions = await self.logger.get_interactions(
            agent_id=agent_id, time_range=time_range, limit=500
        )
    ...
    async def get_collaboration_insights(self) -> Dict[str, Any]:
        ...
        recent_interactions = await self.logger.get_interactions(
            time_range=(time.time() - 3600, time.time()), limit=1000  # Last hour
        )
        ...
        agents = list(
            set(
                [i.source_agent for i in recent_interactions]
                + [i.target_agent for i in recent_interactions if i.target_agent]
            )
        )

        # Build collaboration graph
        collaboration_graph = await self.analyzer._build_collaboration_graph(
            recent_interactions, agents
        )
