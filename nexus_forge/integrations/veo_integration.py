...
class VeoIntegration:
    ...
    async def generate_feature_showcase(
        ...
        request = VideoGenerationRequest(
            prompt=prompt,
            duration=15,
            mockup_urls=[mockup_url] if mockup_url else None
        )
    ...
    async def _call_veo_api(self, request: VideoGenerationRequest) -> Dict[str, Any]:
        ...
        if not self.credentials:
            ...
            return {"operation_name": await self._simulate_veo_generation(request), "status": "simulated"}
        ...
        try:
            ...
            raise ModelTimeoutError(
                "Veo API request timeout",
                model="veo-3.0",
                timeout=120.0
            ) from e
        except aiohttp.ClientError as e:
            raise ServiceUnavailableError(
                f"Network error calling Veo API: {str(e)}",
                service="veo"
            ) from e
        except Exception as e:
    ...
    async def _incorporate_mockups(
        self,
        video_url: str,
        mockup_urls: List[str]
    ) -> str:
...
class VeoVideoEditor:
    ...
    async def check_operation_status(self, operation_name: str) -> Dict[str, Any]:
        ...
        if not self.credentials:
            return {"status": "simulated", "progress": 100, "video_url": "simulation_complete"}
