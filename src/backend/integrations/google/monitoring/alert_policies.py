...
class AlertPolicyManager:
    ...
    def create_notification_channels(self) -> Dict[str, str]:
        ...
        try:
            ...
            slack_created = self.notification_client.create_notification_channel(
                name=self.project_name, notification_channel=slack_channel
            )
            channels["slack"] = slack_created.name

            pagerduty_created = self.notification_client.create_notification_channel(
                name=self.project_name, notification_channel=pagerduty_channel
            )
    ...
    def _create_high_error_rate_policy(
    ...
    def _create_high_memory_policy(
        self, channels: Dict[str, str]
    ) -> monitoring_v3.AlertPolicy:
    ...
    def _create_websocket_connection_policy(
        self, channels: Dict[str, str]
    ) -> monitoring_v3.AlertPolicy:
    ...
    def _create_agent_failure_policy(
        self, channels: Dict[str, str]
    ) -> monitoring_v3.AlertPolicy:
    ...
    def _create_token_usage_policy(
        self, channels: Dict[str, str]
    ) -> monitoring_v3.AlertPolicy:
    ...
    def _create_cache_performance_policy(
        self, channels: Dict[str, str]
    ) -> monitoring_v3.AlertPolicy:
...
ALERT_ROUTING_RULES = {
    ...
    "after_hours": {"channels": ["email"], "critical_only": True},
    "weekends": {"days": ["SAT", "SUN"], "channels": ["email"], "critical_only": True},
}
