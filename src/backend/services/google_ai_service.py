...
class GoogleAIService:
    ...
    async def generate_complete_app(
        ...
        try:
            ...
            demo_video = await self._generate_demo_video(
                app_description, requirements, mockups
            )

            # Step 4: Generate code structure using Gemini Pro
            logger.info("Generating code structure...")
            code_structure = await self._generate_code_structure(
                app_spec, target_platform
            )
    ...
    async def _generate_app_mockups(
        self, app_description: str, app_spec: str
    ) -> Dict[str, Any]:
    ...
    async def _generate_demo_video(
        self, app_description: str, requirements: List[str], mockups: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _generate_code_structure(
        self, app_spec: str, target_platform: str
    ) -> Dict[str, Any]:
            ...
            for phase in phases:
                ...
                for task in phase["tasks"]:
                    prompt = await self.jules.create_jules_task_prompt(
                        task, "feature", {"app_spec": app_spec, "phase": phase["name"]}
                    )
                    phase_prompts.append({"task": task, "jules_prompt": prompt})
    ...
    def _summarize_jules_activity(self, prs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for pr in prs:
            ...
            total_changes += changes.get("total_additions", 0) + changes.get(
                "total_deletions", 0
            )
    ...
    async def optimize_prompts_for_services(
        ...
        for service, prompt in user_prompts.items():
            try:
                ...
                optimized_prompts[service] = (
                    prompt  # Use original if optimization fails
                )
