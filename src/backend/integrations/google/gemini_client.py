...
class GeminiClient:
    ...
    def __init__(
        ...
        self.api_key = api_key or os.getenv(
            "GEMINI_API_KEY", "AIzaSyA5YfZt0dp3ncJ2V-G6iEEhwq9IfjlxkDY"
        )
        ...
        try:
            if not self.api_key:
                raise InvalidAPIKeyError(
                    "Gemini API key not provided", service="gemini"
                )
            ...
            raise InvalidAPIKeyError(
                f"Invalid Gemini API key: {str(e)}", service="gemini"
            ) from e
        except google_exceptions.PermissionDenied as e:
            raise AuthenticationError(
                f"Permission denied for Gemini API: {str(e)}", service="gemini"
            ) from e
        except Exception as e:
            logger.error(f"Failed to initialize Gemini client: {str(e)}")
            raise ServiceUnavailableError(
                f"Failed to initialize Gemini client: {str(e)}", service="gemini"
            ) from e
    ...
    def _handle_gemini_exception(self, e: Exception, operation: str) -> Exception:
        ...
        if isinstance(e, google_exceptions.ResourceExhausted):
            ...
            return InvalidAPIKeyError(
                f"Invalid API key during {operation}: {str(e)}", service="gemini"
            )
        elif isinstance(e, google_exceptions.PermissionDenied):
            return AuthenticationError(
                f"Permission denied during {operation}: {str(e)}", service="gemini"
            )
            ...
            if "too large" in str(e).lower():
                ...
                return ServiceUnavailableError(
                    f"Invalid request during {operation}: {str(e)}", service="gemini"
                )
        else:
            return ServiceUnavailableError(
                f"Gemini service error during {operation}: {str(e)}", service="gemini"
            )
    ...
    async def _generate_standard(
        self, model: genai.GenerativeModel, prompt: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not content:
                raise ServiceUnavailableError(
                    "Empty response from Gemini API", service="gemini"
                )
    ...
    async def create_chat_session(
        self, system_instruction: Optional[str] = None, model_type: str = "flash"
    ) -> genai.ChatSession:
        ...
        try:
            ...
            if system_instruction:
                if (
                    len(system_instruction) > 50000
                ):  # Reasonable limit for system instruction
                    raise PayloadTooLargeError(
    ...
    async def optimize_prompt_for_service(
        self, service_name: str, user_prompt: str, context: Dict[str, Any]
    ) -> str:
...
async def example_gemini_usage():
    ...
    try:
        ...
        response = await asyncio.to_thread(
            chat.send_message, "How can I optimize this database query?"
        )
        print(response.text)
        ...
        print(
            f"Rate limit exceeded: {e.message}. Retry after: {e.details.get('retry_after')} seconds"
        )
