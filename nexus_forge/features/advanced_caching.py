...
from ..core.cache import RedisCache, CacheStrategy, CacheMetrics
...
class AccessPatternAnalyzer:
    ...
    async def predict_next_accesses(self, agent_id: str, model: PredictionModel = PredictionModel.STATISTICAL) -> PredictionResult:
        """
        ...
        for key, stats in key_stats.items():
            if stats["count"] >= 3:  # Minimum accesses for prediction
                ...
                if stats["intervals"]:
                    ...
                    time_confidence = 1.0 / (1.0 + abs(time_to_prediction) / self.prediction_window)
                else:
                    time_confidence = 0.5
    ...
    async def _predict_heuristic(self, agent_id: str) -> PredictionResult:
        ...
        recent_accesses = [a for a in self.agent_patterns.get(agent_id, []) 
                          if time.time() - a.access_time < 3600]  # Last hour
        ...
        for key in predictions:
            for correlated_key in self.key_correlations.get(key, set()):
                if correlated_key not in predictions:
                    ...
                    corr_confidence = base_confidence * 0.7  # Correlated keys have 70% confidence
    ...
    async def _predict_lstm(self, agent_id: str) -> PredictionResult:
        ...
        all_keys = set(statistical_result.keys_to_preload + heuristic_result.keys_to_preload)
        ...
        for key in all_keys:
            ...
            if combined_conf > 0.4:  # Higher threshold for LSTM
                ...
                stat_time = statistical_result.predicted_access_time.get(key, time.time() + 300)
                heur_time = heuristic_result.predicted_access_time.get(key, time.time() + 300)
    ...
    async def get_seasonal_prediction(self, agent_id: str, target_time: float) -> List[str]:
        """
        ...
        for pattern_key, patterns in self.seasonal_patterns.items():
            ...
            hourly_prob = patterns["hourly"][target_hour] / max(patterns["total_accesses"], 1)
            daily_prob = patterns["daily"][target_day] / max(patterns["total_accesses"], 1)
...
class CachePreloader:
    ...
    async def _generate_multi_model_predictions(self, agent_id: str) -> List[PredictionResult]:
        """Generate predictions using multiple models"""
        models = [PredictionModel.STATISTICAL, PredictionModel.HEURISTIC, PredictionModel.LSTM]
    ...
    async def _queue_preload_tasks(self, agent_id: str, predictions: List[PredictionResult]):
        ...
        for prediction in predictions:
            for key in prediction.keys_to_preload:
                ...
                predicted_time = prediction.predicted_access_time.get(key, time.time() + 300)
    ...
    async def _execute_preload(self, task: Dict[str, Any]) -> bool:
        ...
        if predicted_time > current_time:
            wait_time = min(predicted_time - current_time - 30, 300)  # Preload 30s early, max wait 5min
            if wait_time > 0:
        ...
        try:
            ...
            if preload_data is not None:
                ...
                if success:
                    logger.info(f"Successfully preloaded key {key} for agent {agent_id}")
                    
                    # Store preload metadata
                    await self.cache.set_l3(f"preload_meta:{key}", {
                        "agent_id": agent_id,
    ...
    async def _analyze_preload_effectiveness(self):
        ...
        recent_accesses = [a for a in self.analyzer.access_history 
                          if time.time() - a.access_time < 300 and a.hit]  # Last 5 minutes
        ...
        if total_checked > 0:
            ...
            logger.info(f"Preload effectiveness: {preload_hit_rate:.2%} of cache hits were preloaded")
...
class IntelligentCacheWarmer:
    ...
    async def create_warming_strategy(self, agent_id: str, optimization_strategy: CacheOptimizationStrategy) -> CacheWarmingStrategy:
    ...
    async def _create_recency_strategy(self, agent_id: str) -> CacheWarmingStrategy:
        ...
        recent_accesses = [a for a in self.analyzer.agent_patterns.get(agent_id, []) 
        ...
        for i, key in enumerate(recent_keys[:15]):
            ...
            warming_schedule[key] = current_time + (i * 30) + 300  # Every 30s, starting in 5min
    ...
    async def _create_predictive_strategy(self, agent_id: str) -> CacheWarmingStrategy:
        ...
        predictions = await self.analyzer.predict_next_accesses(agent_id, PredictionModel.LSTM)
        
        # Filter high-confidence predictions
        high_confidence_keys = [
            key for key, confidence in predictions.confidence_scores.items()
            if confidence > 0.6
        ]
        ...
        for key in high_confidence_keys:
            predicted_time = predictions.predicted_access_time.get(key, time.time() + 600)
            warm_time = predicted_time - 300  # Warm 5 minutes before predicted access
            warming_schedule[key] = max(warm_time, time.time() + 60)  # At least 1 minute from now
    ...
    async def _create_correlation_strategy(self, agent_id: str) -> CacheWarmingStrategy:
        ...
        recent_keys = [a.key for a in self.analyzer.agent_patterns.get(agent_id, [])[-20:]]
        ...
        for i, key in enumerate(priority_keys[:12]):
            ...
            warming_schedule[key] = current_time + (i * 45) + 180  # Every 45s, starting in 3min
    ...
    async def _create_performance_strategy(self, agent_id: str) -> CacheWarmingStrategy:
        ...
        for i, key in enumerate(priority_keys[:10]):
            ...
            warming_schedule[key] = current_time + (i * 20) + 120  # Every 20s, starting in 2min
    ...
    async def execute_warming_strategy(self, agent_id: str, strategy: CacheWarmingStrategy):
        ...
        logger.info(f"Warming strategy completed: {successful_warms}/{total_warms} successful")
        
        return warming_result
    
    async def _warm_key_at_time(self, key: str, warm_time: float, agent_id: str) -> bool:
        ...
        try:
            ...
            if warm_data is not None:
                ...
                if success:
                    ...
                    # Mark as warmed
                    await self.cache.set_l3(f"warmed:{key}", {
                        "agent_id": agent_id,
                        "warm_time": time.time(),
                        "ttl": ttl
                    }, ttl)
...
class AdvancedCacheOrchestrator:
    ...
    async def record_cache_access(self, key: str, agent_id: str, hit: bool, size: int = 0, ttl: int = 0):
        """Record cache access for pattern analysis"""
        ...
        while self.optimization_active:
            try:
                ...
                if baseline_metrics is None:
                    ...
                    self.performance_metrics["cache_hit_rate_improvement"] = hit_rate_improvement
                    ...
                    if hit_rate_improvement > 5:  # 5% improvement
                        logger.info(f"Cache hit rate improved by {hit_rate_improvement:.1f}%")
    ...
    async def _detect_emerging_patterns(self) -> Dict[str, List[str]]:
        ...
        recent_accesses = [a for a in self.analyzer.access_history if a.access_time > cutoff_time]
    ...
    async def _create_adaptive_warming(self, agent_id: str, recent_keys: List[str]):
        ...
        for i, key in enumerate(recent_keys[:8]):  # Top 8 recent keys
            ...
            warming_schedule[key] = current_time + (i * 20) + 60  # Every 20s, starting in 1min
    ...
    async def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pattern_analysis": {
                ...
                "correlation_count": sum(len(corr) for corr in self.analyzer.key_correlations.values()),
                "seasonal_patterns": len(self.analyzer.seasonal_patterns)
            },
            "optimization_active": self.optimization_active
        }
    
    async def optimize_for_agent(self, agent_id: str, strategy: CacheOptimizationStrategy = CacheOptimizationStrategy.PERFORMANCE_DRIVEN) -> Dict[str, Any]:
        """Manually trigger optimization for specific agent"""
        logger.info(f"Manual optimization triggered for agent {agent_id} with strategy {strategy.value}")
...
async def record_cache_access(key: str, agent_id: str, hit: bool, size: int = 0, ttl: int = 0):
    """Record cache access for optimization"""
    orchestrator = await get_advanced_cache_orchestrator()
    await orchestrator.record_cache_access(key, agent_id, hit, size, ttl)


async def optimize_cache_for_agent(agent_id: str, strategy: CacheOptimizationStrategy = CacheOptimizationStrategy.PERFORMANCE_DRIVEN):
...
async def main():
    ...
    await orchestrator.record_cache_access("api_response:users", "starri_orchestrator", True, 1024, 3600)
    await orchestrator.record_cache_access("code_gen:python:function", "jules_coding_agent", False, 2048, 1800)
    await orchestrator.record_cache_access("mockup:dashboard:main", "imagen_4_designer", True, 5120, 7200)
    ...
    result = await orchestrator.optimize_for_agent("jules_coding_agent", CacheOptimizationStrategy.PREDICTIVE)
