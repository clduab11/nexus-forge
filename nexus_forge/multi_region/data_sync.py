...
from .models import (
    Region, DataSyncConfig, ReplicationMode, 
    RegionStatus, HealthStatus
)
...
class DataSynchronizer:
    ...
    async def list_sync_configs(self) -> List[DataSyncConfig]:
        ...
        result = await self.supabase.client.table("data_sync_configs") \
            .select("*") \
            .execute()
    ...
    async def get_sync_status(self, config_id: str) -> Dict[str, Any]:
        ...
        task_running = config_id in self._sync_tasks and not self._sync_tasks[config_id].done()
        ...
        return {
            ...
            "health_status": "healthy" if failed_ops == 0 and lag_seconds < 600 else "warning"
        }
    ...
    async def get_conflicts(
        ...
        for conflict_data in result.data:
            conflicts.append(ConflictRecord(
                ...
                source_timestamp=datetime.fromisoformat(conflict_data["source_timestamp"]),
                target_timestamp=datetime.fromisoformat(conflict_data["target_timestamp"]),
    ...
    async def resolve_conflict(
        ...
        # Get conflict record
        result = await self.supabase.client.table("sync_conflicts") \
            .select("*") \
            .eq("id", conflict_id) \
            .execute()
        ...
        try:
            ...
            if strategy == ConflictResolution.MANUAL:
                ...
                resolved_data = conflict_data["source_data"] if source_ts > target_ts else conflict_data["target_data"]
    ...
    async def get_sync_metrics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        result = await self.supabase.client.table("sync_operations") \
            .select("*") \
        ...
        for op in operations:
            ...
            metrics["operations_by_table"][table] = metrics["operations_by_table"].get(table, 0) + 1
        
        # Group by region
        for op in operations:
            region = op.get("target_region_id", "unknown")
            metrics["operations_by_region"][region] = metrics["operations_by_region"].get(region, 0) + 1
        
        # Calculate error rate
        failed_ops = metrics["operations_by_status"].get("failed", 0)
        if metrics["total_operations"] > 0:
            metrics["error_rate_percent"] = (failed_ops / metrics["total_operations"]) * 100
        
        # Get conflict metrics
        conflict_result = await self.supabase.client.table("sync_conflicts") \
            .select("*") \
        ...
        while self._running:
            try:
                ...
                for config in configs:
                    # Check if sync task is running
                    if config.id not in self._sync_tasks or self._sync_tasks[config.id].done():
                        # Start sync task
                        await self._start_sync_task(config)
                
                # Clean up completed tasks
                completed_tasks = [
                    config_id for config_id, task in self._sync_tasks.items()
                    if task.done()
                ]
        ...
        while self._running:
            try:
                ...
                if operations:
                    ...
                    await self.supabase.client.table("data_sync_configs") \
                        .update({"last_sync": config.last_sync.isoformat()}) \
                        .eq("id", config.id) \
                        .execute()
