...
class KnowledgeGraphAgent(Agent):
    ...
    def generate_knowledge_graph(self, request: Dict) -> Dict:
        ...
        try:
            ...
            if not summary and not sources and not focus_areas:
                self.logger.warning(
                    "Insufficient information to generate knowledge graph"
                )
                return self._create_empty_graph_response(request_id)

            # Extract entities and relationships
            entities, relationships = self._extract_entities_relationships(
                query, summary, sources, focus_areas
            )

            # Create knowledge graph
            graph = self._create_knowledge_graph(
                query, entities, relationships, max_nodes
            )
    ...
    def _extract_entities_relationships(
        self, query: str, summary: str, sources: List[Dict], focus_areas: List[Dict]
        ...
        try:
            ...
            if focus_areas:
                ...
                for i, area in enumerate(focus_areas[:5], 1):
                    area_content = (
                        area.get("area", "") if isinstance(area, dict) else area
                    )
            ...
            try:
                ...
                json_match = re.search(
                    r"```json\s*(.*?)\s*```", extraction_result, re.DOTALL
                )
                if json_match:
    ...
    def _create_fallback_entities_relationships(
        self, query: str
    ) -> Tuple[List[Dict], List[Dict]]:
        """
        ...
        """
        self.logger.info(
            f"Creating fallback entities and relationships for query: {query}"
        )
    ...
    def _create_knowledge_graph(
        ...
        try:
            ...
            for entity in entities:
                if (
                    entity.get("label", "").lower() in query.lower()
                    or query.lower() in entity.get("label", "").lower()
                ):
                    main_topic_id = entity.get("id")
                    entity["confidence"] = (
                        1.0  # Ensure main topic has highest confidence
                    )
                    break

            # If no main topic found, use the highest confidence entity
            if not main_topic_id and entities:
                main_topic_id = max(entities, key=lambda e: e.get("confidence", 0)).get(
                    "id"
                )
            ...
            return {"nodes": [], "edges": [], "main_topic": "", "query": query}
    ...
    def _generate_graph_summary(self, graph: Dict, query: str) -> str:
        ...
        try:
            ...
            main_topic_edges = [
                e
                for e in edges
                if e.get("source") == main_topic or e.get("target") == main_topic
            ]
            main_topic_edges = sorted(
                main_topic_edges, key=lambda e: e.get("weight", 0), reverse=True
            )
            ...
            for edge in main_topic_edges[:5]:  # List top 5 connections
                ...
                connected_node = next(
                    (n for n in nodes if n.get("id") == connected_id), None
                )
                ...
                if connected_node:
                    ...
                    if source_id == main_topic:
                        summary += (
                            f"- {main_topic_label} {relation_label} {connected_label}\n"
                        )
                    else:
                        summary += (
                            f"- {connected_label} {relation_label} {main_topic_label}\n"
                        )
    ...
    def _create_empty_graph_response(self, request_id: str) -> Dict:
        ...
        return {
            "request_id": request_id,
            "graph": {"nodes": [], "edges": [], "main_topic": "", "query": ""},
