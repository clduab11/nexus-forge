...
REQUEST_COUNT = Counter('nexus_forge_requests_total', 'Total HTTP requests', ['method', 'endpoint', 'status', 'user_tier'])
REQUEST_LATENCY = Histogram('nexus_forge_request_duration_seconds', 'HTTP request latency', ['method', 'endpoint'])
RESEARCH_TASKS = Counter('nexus_forge_research_tasks_total', 'Total research tasks', ['status', 'complexity'])
ERROR_COUNT = Counter('nexus_forge_errors_total', 'Total errors', ['type', 'service', 'severity'])

# AI-specific metrics
AI_SERVICE_ERRORS = Counter('nexus_forge_ai_errors_total', 'AI service errors', ['service', 'error_type', 'model'])
AI_REQUESTS = Counter('nexus_forge_ai_requests_total', 'AI service requests', ['service', 'model', 'operation'])
AI_MODEL_LATENCY = Histogram('nexus_forge_ai_request_duration_seconds', 'AI model response latency', ['service', 'model', 'operation'])
TOKEN_USAGE = Counter('nexus_forge_tokens_used_total', 'Total tokens consumed', ['service', 'model', 'type'])
TOKEN_QUOTA_USAGE = Gauge('nexus_forge_token_quota_usage_percent', 'Token quota usage percentage', ['service'])

# System metrics
RETRY_COUNT = Counter('nexus_forge_retry_attempts_total', 'Retry attempts', ['service', 'function'])
CIRCUIT_BREAKER_STATE = Gauge('nexus_forge_circuit_breaker_state', 'Circuit breaker state (0=closed, 1=half_open, 2=open)', ['service'])
ERROR_RATE = Gauge('nexus_forge_error_rate_percentage', 'Error rate percentage', ['service', 'time_window'])
SERVICE_AVAILABILITY = Gauge('nexus_forge_service_availability_percentage', 'Service availability percentage', ['service'])
QUOTA_USAGE = Gauge('nexus_forge_quota_usage_percentage', 'API quota usage percentage', ['service', 'quota_type'])

# Business metrics
ACTIVE_USERS = Gauge('nexus_forge_active_users', 'Number of active users')
CONCURRENT_REQUESTS = Gauge('nexus_forge_concurrent_requests', 'Number of concurrent requests')
...
# Redis metrics
REDIS_COMMANDS = Counter('nexus_forge_redis_commands_total', 'Redis commands executed', ['command', 'status'])
REDIS_CONNECTION_POOL = Gauge('nexus_forge_redis_connection_pool_size', 'Redis connection pool size')
REDIS_CONNECTION_FAILURES = Counter('nexus_forge_redis_connection_failures_total', 'Redis connection failures')
REDIS_LATENCY = Histogram('nexus_forge_redis_latency_seconds', 'Redis operation latency', ['operation'])

# Health checks
HEALTH_CHECK_STATUS = Gauge('nexus_forge_health_check_status', 'Health check status (1=healthy, 0=unhealthy)', ['check_name', 'service'])
HEALTH_CHECK_LATENCY = Histogram('nexus_forge_health_check_duration_seconds', 'Health check latency', ['check_name'])

# Database metrics
DB_CONNECTION_POOL_ACTIVE = Gauge('nexus_forge_db_connection_pool_active', 'Active database connections')
DB_CONNECTION_POOL_SIZE = Gauge('nexus_forge_db_connection_pool_size', 'Total database connection pool size')
DB_QUERY_DURATION = Histogram('nexus_forge_db_query_duration_seconds', 'Database query duration', ['query_type'])

# Cache-specific metrics
CACHE_HIT_RATE = Gauge('nexus_forge_cache_hit_rate_percentage', 'Cache hit rate percentage', ['service', 'cache_type'])
CACHE_OPERATIONS = Counter('nexus_forge_cache_operations_total', 'Total cache operations', ['operation', 'service', 'status'])
CACHE_LATENCY = Histogram('nexus_forge_cache_operation_latency_seconds', 'Cache operation latency', ['operation', 'service'])
CACHE_SIZE = Gauge('nexus_forge_cache_size_bytes', 'Cache size in bytes', ['service'])
CACHE_EVICTIONS = Counter('nexus_forge_cache_evictions_total', 'Total cache evictions', ['service', 'reason'])
CACHE_TTL_EFFECTIVENESS = Histogram('nexus_forge_cache_ttl_effectiveness_ratio', 'Ratio of actual vs intended TTL', ['service', 'cache_type'])
...
class ErrorMetrics:
    ...
    def _cleanup_old_timestamps(self, service: str):
        ...
        while (self.error_timestamps[service] and 
               self.error_timestamps[service][0] < cutoff):
            self.error_timestamps[service].popleft()
        
        # Clean success timestamps
        while (self.success_timestamps[service] and 
               self.success_timestamps[service][0] < cutoff):
    ...
    def get_error_rate(self, service: str) -> float:
        ...
        with self.lock:
            ...
            ERROR_RATE.labels(service=service, time_window=f"{self.window_size}s").set(rate)
            return rate
...
class APILogger:
    ...
    def log_error(self, error_type: str, error: Exception, service: Optional[str] = None):
    ...
    def log_cache_hit(self, service: str, cache_type: str, hit_rate: float):
        ...
        CACHE_OPERATIONS.labels(operation="hit", service=service, status="success").inc()
    
    def log_cache_miss(self, service: str, cache_type: str):
        """Log cache miss metrics"""
        CACHE_OPERATIONS.labels(operation="miss", service=service, status="success").inc()
    ...
    def log_cache_operation(
        ...
        CACHE_OPERATIONS.labels(operation=operation, service=service, status=status).inc()
    ...
    def log_cache_ttl_effectiveness(
        ...
        CACHE_TTL_EFFECTIVENESS.labels(
            service=service, 
            cache_type=cache_type
        ).observe(effectiveness_ratio)
    ...
    def log(self, level: str, message: str, **kwargs):
        ...
        if level == "error":
            api_logger.error_metrics.record_error(
                self.service_name, 
                kwargs.get("error_type", "unknown_error")
            )
        ...
        **kwargs
        ...
        if success:
            ...
            api_logger.log_ai_service_error(
                self.service_name, error_type, model, error or Exception("Unknown error"), operation
            )
            log_data["error"] = str(error) if error else "Unknown error"
            self.log("error", f"AI operation failed: {operation}", error_type=error_type, **log_data)
...
def _generate_cache_recommendations_internal(stats: Dict[str, Any]) -> list:
    ...
    if hit_rate < 50:
        ...
        recommendations.append("Excellent cache performance - consider expanding to more endpoints")
    
    evictions = stats.get('evictions', 0)
    if evictions > 100:
        recommendations.append("High eviction rate - consider increasing cache size or optimizing data")
...
def monitor_cache_performance():
    ...
    try:
        ...
        if health["status"] == "healthy":
            ...
            logger.error(f"Cache health check failed: {health.get('error', 'Unknown error')}")
