...
class GoogleSearchTool:
    ...
    def __init__(
        ...
        if self.mock_mode:
            logger.warning(
                "Google Search API credentials not found. Running in mock mode."
            )
        else:
    ...
    async def search_academic(
        ...
        return await self.search(
            query=academic_query, num_results=num_results, safe_search="active"
        )

    async def search_news(
        self, query: str, num_results: int = 10, recency: str = "w1"
    ) -> Dict[str, Any]:
        """
    ...
    async def search_specific_sites(
        self, query: str, sites: List[str], num_results: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "search_metadata": {"sites_searched": sites, "search_type": "multi_site"},
        }

    async def _process_search_results(
        self, search_data: Dict, query: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "query": query,
                "total_results": search_data.get("searchInformation", {}).get(
                    "totalResults", 0
                ),
                "search_time": search_data.get("searchInformation", {}).get(
                    "searchTime", 0
                ),
                ...
                "search_metadata": {
                    ...
                    "safe_search": "active",
                    "spelling_suggestions": search_data.get("spelling", {}).get(
                        "correctedQuery", ""
                    ),
                },
            }
    ...
    def _classify_source_type(self, url: str) -> str:
        ...
        if ".edu" in url_lower or "scholar.google" in url_lower:
            ...
            news_site in url_lower
            for news_site in ["news", "reuters", "bbc", "cnn", "nytimes"]
        ):
            return "news"
            ...
            social in url_lower
            for social in ["reddit", "twitter", "facebook", "linkedin"]
        ):
            return "social"
    ...
    async def _mock_search(
        self, query: str, num_results: int, error: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            mock_items.append(
                {
                    ...
                    "source_type": ["academic", "news", "general", "organization"][
                        i % 4
                    ],
