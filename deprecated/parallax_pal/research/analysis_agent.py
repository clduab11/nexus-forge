...
class AnalysisAgent(Agent):
    ...
    async def synthesize_information(
        self, topic: str, sources: List[Dict[str, Any]], patterns: List[str] = None
    ) -> Tuple[str, List[str]]:
        """
    ...
    async def _generate_summary(
        ...
        medium_rel_sources = [
            cp for cp in content_pieces if 0.4 <= cp["reliability"] <= 0.7
        ]
    ...
    async def _extract_key_points(
        ...
        for content in content_pieces:
            ...
            for sentence in sentences:
                ...
                relevance_score = sum(
                    1 for keyword in topic_keywords if keyword in sentence_lower
                )

                if (
                    relevance_score > 0 and len(sentence) > 20
        ...
        while len(key_points) < self.min_key_points and len(key_points) < len(
            content_pieces
        ):
            source_title = content_pieces[len(key_points)].get(
                "title", f"Source {len(key_points)+1}"
            )
    ...
    def _generate_pattern_insights(
        self, topic: str, patterns: List[str], content_pieces: List[Dict[str, Any]]
    ) -> List[str]:
        """
        ...
        for pattern in patterns:
            if pattern == "comparison":
                insights.append(
                    f"Comparative analysis of {topic} shows distinct characteristics across different contexts."
                )
            elif pattern == "trends":
                insights.append(
                    f"Current trends in {topic} indicate ongoing developments and evolving perspectives."
                )
            elif pattern == "impact":
                insights.append(
                    f"The impact of {topic} extends across multiple domains and stakeholder groups."
                )
            elif pattern == "methods":
                insights.append(
                    f"Various methodological approaches to {topic} are documented in the literature."
                )
            elif pattern == "challenges":
                insights.append(
                    f"Several challenges and limitations regarding {topic} are highlighted in current research."
                )
            elif pattern == "benefits":
                insights.append(
                    f"Multiple benefits and advantages of {topic} are identified across different applications."
                )
    ...
    async def generate_followup_questions(
        ...
        try:
            ...
            if "different" in summary.lower() or "various" in summary.lower():
                questions.append(
                    f"What are the key differences in approaches to {topic}?"
                )
