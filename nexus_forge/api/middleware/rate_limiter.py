...
class WebSocketRateLimiter:
    ...
    async def check_websocket_limit(
        self, user_id: str, max_connections: int = 5, max_messages_per_minute: int = 100
    ) -> tuple[bool, str]:
        """
        ...
        allowed, _ = await self.rate_limiter.check_rate_limit(
            f"ws_messages:{user_id}", max_messages_per_minute, 60  # 1 minute window
        )
...
class OperationRateLimiter:
    ...
    async def check_operation_limit(
        self, user_id: str, operation: str, tier: str = "free"
    ) -> tuple[bool, dict]:
        ...
        allowed, metadata = await self.rate_limiter.check_rate_limit(key, limit, window)
