...
from ...core.google_cloud_auth import GoogleCloudConfig, AIServiceError
...
class JulesAgent:
    ...
    async def _execute_with_monitoring(
        ...
        monitor_task = asyncio.create_task(
            self._monitor_task_progress(task, result)
        )
        ...
        try:
            ...
            execution_result = await client.execute_task(
                task,
                auto_commit=not self.config.auto_approve_prs
            )
        ...
        if result.test_coverage:
            ...
            if completed > 0:
                ...
                self._metrics["average_test_coverage"] = (
                    (current_avg * (completed - 1) + result.test_coverage) / completed
                )
