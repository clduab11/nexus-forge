...
class CompiledWorkflow:
    ...
    def get_next_nodes(
        self, current_node: str, execution_result: Any = None, error: bool = False
    ) -> List[str]:
        ...
        for connection in self.connections[current_node]:
            ...
            if error and conn_type == ConnectionType.ERROR:
                should_take = True
            elif not error and conn_type in [
    ...
    def _evaluate_condition(self, condition: str, context: Any) -> bool:
        ...
        try:
            ...
            eval_context = {"result": context, "data": context, "output": context}
...
class WorkflowCompiler:
    ...
    def _validate_graph(self, graph: nx.DiGraph, workflow: Workflow) -> None:
        ...
        if not nx.is_directed_acyclic_graph(graph):
            ...
            for cycle in cycles[:3]:  # Show first 3 cycles
                cycle_names.append(
                    " -> ".join(node_mapping[node_id] for node_id in cycle)
                )

            raise ValidationError(f"Workflow contains cycles: {', '.join(cycle_names)}")
    ...
    def _find_entry_points(self, workflow: Workflow) -> List[str]:
        ...
        if not entry_points:
            trigger_nodes = [
                node.id for node in workflow.nodes if node.type == NodeType.TRIGGER
            ]
    ...
    def generate_execution_plan(
        self, compiled_workflow: CompiledWorkflow
    ) -> Dict[str, Any]:
        ...
        for entry_id in compiled_workflow.entry_points:
            ...
            plan["entry_points"].append(
                {"id": entry_id, "name": node.name, "type": node.type.value}
            )

        # Generate execution paths
        for entry_id in compiled_workflow.entry_points:
            path = self._trace_execution_path(compiled_workflow, entry_id)
            plan["execution_paths"].append({"start": entry_id, "nodes": path})
    ...
    def _trace_execution_path(
        ...
        while queue and len(path) < 100:  # Prevent infinite loops
            ...
            path.append({"id": current, "name": node.name, "type": node.type.value})
