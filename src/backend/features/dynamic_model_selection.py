...
class TaskComplexityAnalyzer:
    ...
    async def analyze_task(
        self, task_text: str, context: Optional[Dict[str, Any]] = None
    ) -> TaskFeatures:
        """
        ...
        try:
            ...
            if self.nlp_available:
                ...
                sentences = [
                    s.strip() for s in re.split(r"[.!?]+", task_text) if s.strip()
                ]
            ...
            reasoning_depth = await self._calculate_reasoning_depth(
                task_text, sentences
            )
            ...
            context_requirements = await self._estimate_context_requirements(
                task_text, context
            )
    ...
    async def _calculate_complexity_score(
        ...
        try:
            ...
            avg_syllables = sum(self._count_syllables(word) for word in words) / max(
                len(words), 1
            )

            # Sentence complexity (nested clauses, conjunctions)
            nested_clauses = sum(
                text.count(conj) for conj in ["that", "which", "who", "where", "when"]
            )
            coordination = sum(
                text.count(conj)
                for conj in ["and", "but", "or", "yet", "for", "nor", "so"]
            )
            ...
            technical_terms = sum(
                1
                for word in words
                if len(word) > 8 and word.endswith(("tion", "sion", "ment", "ness"))
            )
            ...
            complexity = min(
                (
                    ...
                    + nested_clauses
                    / max(len(sentences), 1)
                    * 0.25  # Clause complexity
                    + coordination
                    / max(len(sentences), 1)
                    * 0.15  # Coordination complexity
    ...
    async def _calculate_reasoning_depth(self, text: str, sentences: List[str]) -> int:
        ...
        try:
            ...
            reasoning_count = sum(
                text.lower().count(indicator) for indicator in self.reasoning_indicators
            )
            ...
            conditional_count = sum(
                text.lower().count(cond)
                for cond in ["if", "unless", "provided", "assuming"]
            )

            # Count comparative language
            comparative_count = sum(
                text.lower().count(comp)
                for comp in ["better", "worse", "more", "less", "compared"]
            )

            # Calculate depth score
            total_indicators = (
                reasoning_count + question_count + conditional_count + comparative_count
            )
    ...
    async def _classify_domain(self, words: List[str], text: str) -> TaskDomain:
        ...
        try:
            ...
            if any(
                word in text.lower()
                for word in ["design", "ui", "ux", "visual", "mockup"]
            ):
                domain_scores["design"] = domain_scores.get("design", 0) + 2

            if any(
                word in text.lower()
                for word in ["code", "function", "api", "debug", "implement"]
            ):
    ...
    async def _estimate_context_requirements(
        ...
        try:
            ...
            if any(
                ...
                for phrase in ["refer to", "based on", "according to", "considering"]
            ):
                requirement += 1
            ...
            if any(
                phrase in text.lower()
                for phrase in ["coordinate", "integrate", "combine", "merge"]
            ):
                requirement += 1
            ...
            if any(
                phrase in text.lower()
                for phrase in ["long-term", "strategy", "roadmap", "timeline"]
            ):
                requirement += 1
    ...
    async def _calculate_creativity_score(self, words: List[str], text: str) -> float:
        ...
        try:
            creativity_count = sum(
                1 for word in words if word in self.creativity_indicators
            )

            # Check for open-ended questions
            open_ended = sum(
                text.lower().count(phrase)
                for phrase in ["what if", "imagine", "create", "design"]
            )

            # Check for artistic/creative terms
            artistic_terms = sum(
                text.lower().count(term)
                for term in ["art", "creative", "innovative", "original"]
            )
            ...
            creativity_score = min(
                total_creative_indicators / max(total_words, 1) * 3, 1.0
            )
    ...
    async def _track_analysis_performance(self, analysis_time_ms: float):
        ...
        try:
            await self.cache.set_l1(
                f"analysis_performance:{int(time.time())}",
                {"analysis_time_ms": analysis_time_ms, "timestamp": time.time()},
            )
        except Exception as e:
...
class ModelCapabilityMatrix:
    ...
    def _calculate_compatibility_score(
        ...
        try:
            ...
            domain_match = (
                1.0 if task_features.domain in capabilities.domain_strengths else 0.5
            )

            # Feature matching
            reasoning_match = min(
                task_features.reasoning_depth / 5.0, capabilities.reasoning_capability
            )
            creativity_match = min(
                task_features.creativity_score, capabilities.creativity_capability
            )
            technical_match = min(
                task_features.technical_density, capabilities.technical_capability
            )

            # Context requirements
            context_match = (
                1.0
                if task_features.context_requirements * 50000
            ...
            if strategy == SelectionStrategy.SPEED_OPTIMIZED:
                speed_factor = 1.0 / (
                    capabilities.avg_latency_ms / 1000.0
                )  # Inverse latency
                compatibility = (
                    domain_match * 0.2
                    + reasoning_match * 0.1
                ...
                cost_factor = (
                    1.0 / capabilities.cost_per_1k_tokens
                    if capabilities.cost_per_1k_tokens > 0
                    else 1.0
                )
                compatibility = (
                    domain_match * 0.15
    ...
    async def update_model_performance(
        self, model_type: ModelType, outcome: PerformanceOutcome
    ):
        ...
        try:
            ...
            if len(self.performance_history[model_type]) > 100:
                self.performance_history[model_type] = self.performance_history[
                    model_type
                ][-100:]
    ...
    async def _update_capabilities(self, model_type: ModelType):
        ...
        try:
            if (
                model_type not in self.capabilities
                or not self.performance_history[model_type]
            ):
                return
            ...
            self.capabilities[model_type].quality_score = (
                alpha * avg_quality
                + (1 - alpha) * self.capabilities[model_type].quality_score
            )
...
class MultiArmedBanditSelector:
    ...
    async def select_model_ucb1(
        self,
        suitable_models: List[Tuple[ModelType, float]],
        ...
        try:
            ...
            total_selections = sum(
                self.model_arms[model].get("selections", 0)
            ...
            if total_selections == 0:
                ...
                model, compatibility = (
                    np.random.choice(suitable_models)[0],
                    np.random.choice(suitable_models)[1],
                )
            ...
            for model_type, compatibility in suitable_models:
                ...
                if selections == 0:
                    ...
                    confidence_bound = exploration_factor * math.sqrt(
    ...
    async def select_model_thompson_sampling(
        self, suitable_models: List[Tuple[ModelType, float]]
    ) -> Tuple[ModelType, float]:
...
class PerformancePredictor:
    ...
    async def predict_performance(
        self, task_features: TaskFeatures, model_type: ModelType
    ) -> Dict[str, float]:
        """Predict performance metrics for task-model combination"""
        try:
            # Generate cache key
            feature_hash = hashlib.md5(
                str(task_features.to_vector()).encode()
            ).hexdigest()
            ...
            predictions = await self._calculate_heuristic_predictions(
    ...
    async def _calculate_heuristic_predictions(
        self, task_features: TaskFeatures, model_type: ModelType
    ) -> Dict[str, float]:
        ...
        try:
            ...
            text_length_factor = min(
                task_features.text_length / 1000.0, 2.0
            )  # Cap at 2x

            # Latency prediction
            base_latency = capabilities.avg_latency_ms
            latency_adjustment = (
                1.0 + (complexity_factor * 0.5) + (text_length_factor * 0.3)
            )
            ...
            domain_bonus = (
                0.1 if task_features.domain in capabilities.domain_strengths else 0.0
            )
            complexity_penalty = 0.05 if complexity_factor > 0.8 else 0.0

            predicted_quality = min(
                base_quality + domain_bonus - complexity_penalty, 1.0
            )
...
class DynamicModelSelector:
    ...
    async def select_model(
        ...
        try:
            ...
            task_features = await self.complexity_analyzer.analyze_task(
                task_text, context
            )

            # Step 2: Get suitable models based on capabilities
            suitable_models = self.capability_matrix.get_suitable_models(
                task_features, strategy
            )
            ...
            if not suitable_models:
                ...
                if not available_models:
                    ...
                    if (
                        strategy == SelectionStrategy.LEARNING
                        or np.random.random() < 0.1
                    ):
                        # Use Thompson Sampling for exploration
                        selected_model, confidence_score = (
                            await self.bandit_selector.select_model_thompson_sampling(
                                available_models
                            )
                        )
                        ...
                        selected_model, confidence_score = (
                            await self.bandit_selector.select_model_ucb1(
                                available_models
                            )
                        )
                    ...
                    alternative_models = [
                        (model, score)
                        for model, score in available_models
                        if model != selected_model
                    ][:3]

            # Step 5: Predict performance
            expected_performance = await self.performance_predictor.predict_performance(
                task_features, selected_model
            )
    ...
    async def _filter_available_models(
        self, suitable_models: List[Tuple[ModelType, float]]
    ) -> List[Tuple[ModelType, float]]:
    ...
    async def record_model_failure(self, model_type: ModelType):
        ...
        logger.warning(
            f"Model {model_type.value} failure recorded. Count: {breaker['failures']}"
        )
    ...
    async def record_performance_outcome(self, outcome: PerformanceOutcome):
        ...
        try:
            ...
            await self.capability_matrix.update_model_performance(
                outcome.selected_model, outcome
            )
    ...
    def _calculate_reward(self, outcome: PerformanceOutcome) -> float:
        ...
        try:
            ...
            latency_component = max(
                0, 1.0 - (outcome.actual_latency_ms / max_acceptable_latency)
            )
    ...
    async def _store_selection_decision(self, decision: SelectionDecision):
        ...
        try:
            ...
            await self.cache.set_l2(
                decision_key, asdict(decision), timeout=86400
            )  # 24 hours
        except Exception as e:
    ...
    async def get_selection_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_latency = (
                statistics.mean(self.selection_latencies)
                if self.selection_latencies
                else 0.0
            )
            p95_latency = (
                np.percentile(self.selection_latencies, 95)
                if self.selection_latencies
                else 0.0
            )
            ...
            avg_accuracy = (
                statistics.mean(self.selection_accuracy)
                if self.selection_accuracy
                else 0.0
            )
...
class DynamicModelSelectionOrchestrator:
    ...
    async def select_optimal_model(
        ...
        return await self.model_selector.select_model(
            task_text, context, strategy, user_preferences
        )
    ...
    async def _performance_monitoring(self):
        ...
        while self.active:
            try:
                ...
                if (
                    stats.get("selection_performance", {}).get(
            ...
            try:
                ...
                if len(recent_outcomes) >= 10:
                    ...
                    if quality_variance > 0.1:  # High variance - increase exploration
                        self.model_selector.bandit_selector.exploration_rate = min(
                            0.2,
                            self.model_selector.bandit_selector.exploration_rate + 0.01,
                        )
                    else:  # Low variance - decrease exploration
                        self.model_selector.bandit_selector.exploration_rate = max(
                            0.05,
                            self.model_selector.bandit_selector.exploration_rate - 0.01,
                        )
    ...
    async def _share_performance_insights(self):
        ...
        try:
            ...
            if model_usage:
                ...
                for model, usage_count in model_usage.items():
                    if model in arm_stats:
                        ...
                        if (
                            avg_reward > 0.8 and usage_count < 10
                        ):  # High quality, low usage
            ...
            await self.cache.set_l2(
                "model_selection_insights_for_performance", insights, timeout=3600
            )
    ...
    async def _share_behavior_insights(self):
        ...
        try:
            ...
            insights = {
                ...
                "selection_efficiency": len(
                    [d for d in recent_decisions if d.confidence_score > 0.8]
                )
            ...
            await self.cache.set_l2(
                "model_selection_insights_for_behavior", insights, timeout=3600
            )
    ...
    async def _share_caching_insights(self):
        ...
        try:
            ...
            insights = {
                ...
                "selection_cache_hit_potential": len(frequent_patterns)
                / max(len(task_patterns), 1),
                "timestamp": time.time(),
            }

            await self.cache.set_l2(
                "model_selection_insights_for_caching", insights, timeout=3600
            )
    ...
    async def _system_health_monitoring(self):
        ...
        while self.active:
            try:
                ...
                open_breakers = sum(
                    1
                    for status in circuit_breakers.values()
                ...
                health_score = min(
                    ...
                    + (1.0 - open_breakers / max(len(circuit_breakers), 1))
                    * 0.2,  # Availability factor
                    1.0,
                )
...
async def record_model_execution_outcome(
    task_id: str,
    model_type: ModelType,
    actual_latency_ms: float,
    quality_score: float,
