...
class PerformanceBenchmarks:
    ...
    async def test_concurrent_user_performance(
        self, user_counts: List[int] = None
    ) -> Dict[str, Any]:
            ...
            async def simulate_user():
                ...
                try:
                    ...
                    response = self.client.get(
                        "/api/nexus-forge/projects", headers=self.auth_headers
                    )
                    operations.append(
                        ("get_projects", time.time() - start, response.status_code)
                    )

                    # Get agent status
                    start = time.time()
                    response = self.client.get(
                        "/api/nexus-forge/agents/status", headers=self.auth_headers
                    )
                    operations.append(
                        ("get_agents", time.time() - start, response.status_code)
                    )
                    ...
                    operations.append(
                        ("create_project", time.time() - start, response.status_code)
                    )
                    ...
                    if response.status_code == 201:
                        ...
                        start = time.time()
                        response = self.client.get(
                            f"/api/nexus-forge/projects/{project_id}",
                            headers=self.auth_headers,
                        )
                        operations.append(
                            ("get_project", time.time() - start, response.status_code)
                        )
    ...
    async def test_memory_and_cpu_usage(
    ...
    async def test_app_generation_performance(
        self, project_count: int = 5
    ) -> Dict[str, Any]:
                    ...
                    while time.time() - generation_start < max_wait_time:
                        ...
                        if response.status_code == 200:
                            ...
                            if updated_project["status"] == "completed":
                                ...
                                generation_metrics["generation_times"].append(
                                    generation_time
                                )
                                generation_metrics["fastest_generation"] = min(
                                    generation_metrics["fastest_generation"],
                                    generation_time,
                                )
                                generation_metrics["slowest_generation"] = max(
                                    generation_metrics["slowest_generation"],
                                    generation_time,
                                )
        ...
        if generation_metrics["generation_times"]:
            generation_metrics["avg_generation_time"] = statistics.mean(
                generation_metrics["generation_times"]
            )
            generation_metrics["median_generation_time"] = statistics.median(
                generation_metrics["generation_times"]
            )
    ...
    async def test_websocket_performance(
        self, connection_count: int = 50, message_count: int = 100
    ) -> Dict[str, Any]:
        ...
        if websocket_metrics["message_latencies"]:
            websocket_metrics["avg_latency"] = statistics.mean(
                websocket_metrics["message_latencies"]
            )
            websocket_metrics["max_latency"] = max(
                websocket_metrics["message_latencies"]
            )
            websocket_metrics["p95_latency"] = (
                statistics.quantiles(websocket_metrics["message_latencies"], n=20)[18]
                if len(websocket_metrics["message_latencies"]) >= 20
                else websocket_metrics["max_latency"]
            )

        if websocket_metrics["connection_times"]:
            websocket_metrics["avg_connection_time"] = statistics.mean(
                websocket_metrics["connection_times"]
            )
            websocket_metrics["max_connection_time"] = max(
                websocket_metrics["connection_times"]
            )

        websocket_metrics["connection_success_rate"] = (
            websocket_metrics["successful_connections"] / connection_count
        )
    ...
    async def test_database_performance(self) -> Dict[str, Any]:
        ...
        if db_metrics["write_operations"]:
            db_metrics["avg_write_time"] = statistics.mean(
                db_metrics["write_operations"]
            )
...
"""
    ...
    def _format_performance_comparison(self) -> str:
        ...
        if self.metrics.get("generation_times", {}).get("avg_generation_time"):
            ...
            lines.append(
                f"- App Generation Time: {actual:.1f}s (target: {target}s) {status}"
            )

        # API response time
        if self.metrics.get("api_response_times"):
            avg_times = [
                times["avg"] for times in self.metrics["api_response_times"].values()
            ]
            if avg_times:
                ...
                status = "✓" if actual <= target else "✗"
                lines.append(
                    f"- API Response Time: {actual:.3f}s (target: {target}s) {status}"
                )
        ...
        if self.metrics.get("websocket_latencies", {}).get("avg_latency"):
            ...
            status = "✓" if actual <= target else "✗"
            lines.append(
                f"- WebSocket Latency: {actual*1000:.1f}ms (target: {target*1000}ms) {status}"
            )
    ...
    def _generate_recommendations(self) -> str:
        ...
        if self.metrics.get("memory_usage", {}).get("memory_growth", 0) > 500:
            recommendations.append(
                "- Investigate memory leaks (significant memory growth detected)"
            )

        # Check generation success rate
        if self.metrics.get("generation_times", {}).get("success_rate", 1) < 0.9:
            recommendations.append(
                "- Improve app generation reliability (success rate below 90%)"
            )
        ...
        if self.metrics.get("concurrent_user_performance"):
            high_error_rates = [
                f"{users} users"
                for users, data in self.metrics["concurrent_user_performance"].items()
                if data["error_rate"] > 0.1
            ]
            if high_error_rates:
                recommendations.append(
                    f"- Improve error handling for concurrent loads: {', '.join(high_error_rates)}"
                )

        if not recommendations:
            recommendations.append(
                "- Performance meets all targets. Consider load testing with higher user counts."
            )
