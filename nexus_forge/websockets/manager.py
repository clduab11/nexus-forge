...
class WebSocketManager:
    ...
    def __init__(self, supabase_client: Optional[SupabaseCoordinationClient] = None):
        ...
        logger.info("Enhanced Nexus Forge WebSocket manager initialized with Supabase integration")
    ...
    async def setup_realtime_subscriptions(self, session_id: str, user_id: str):
        ...
        try:
            ...
            coordination_subscription = await self.supabase_client.subscribe_to_coordination_events(
            ...
            project_subscription = await self.supabase_client.subscribe_to_project_updates(
                user_id=user_id,
                callback=lambda event: asyncio.create_task(
            ...
            logger.error(f"Failed to setup real-time subscriptions for {session_id}: {e}")
    ...
    async def _handle_coordination_event(self, session_id: str, event: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error handling coordination event for session {session_id}: {e}")
    ...
    async def _handle_agent_state_change(self, session_id: str, event: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error handling agent state change for session {session_id}: {e}")
    ...
    async def _cleanup_realtime_subscriptions(self, session_id: str):
        ...
        if session_id in self.realtime_subscriptions:
            try:
                ...
                logger.info(f"Cleaned up real-time subscriptions for session {session_id}")
                
            except Exception as e:
                logger.error(f"Error cleaning up subscriptions for session {session_id}: {e}")
