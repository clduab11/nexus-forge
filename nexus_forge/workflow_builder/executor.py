...
from .models import (
    WorkflowExecution, NodeExecutionResult, ExecutionStatus,
    ...
    async def execute_node(
        ...
        try:
            ...
            if node.config.retries > result.retry_count:
                logger.info(f"Retrying node {node.id}, attempt {result.retry_count + 1}")
...
class WorkflowExecutor:
    ...
    async def _execute_workflow_internal(
        ...
        while current_nodes and not context.should_stop:
            ...
            # Notify progress
            await self._notify_execution_progress(execution, completed_nodes, current_nodes)
    ...
    async def _execute_node_with_context(
        ...
        input_data = await self._gather_node_inputs(
            node_id, context, compiled_workflow
        )
    ...
    async def _gather_node_inputs(
        ...
        for source, target, edge_data in compiled_workflow.execution_graph.edges(data=True):
            if target == node_id:
    ...
    async def _notify_execution_resume(self, execution: WorkflowExecution):
        ...
        await self.websocket_manager.broadcast({
            "type": "workflow_execution_resume",
            "execution_id": execution.id
        })
