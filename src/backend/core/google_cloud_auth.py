...
class GoogleCloudAuth:
            ...
            try:
                self.credentials = (
                    service_account.Credentials.from_service_account_file(
                        service_account_path, scopes=self._get_required_scopes()
                    )
                )
            ...
            try:
                ...
                self.credentials = (
                    service_account.Credentials.from_service_account_info(
                        service_account_info, scopes=self._get_required_scopes()
                    )
                )
    ...
    def validate_credentials(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "error": str(e), "auth_method": self.auth_method}
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vertex_ai": {
                ...
                "enable_streaming": os.environ.get(
            ...
            "imagen": {
                ...
                "default_resolution": os.environ.get(
            ...
            "jules": {
                ...
                "auto_approve_prs": os.environ.get(
            ...
            "rate_limiting": {
                "requests_per_minute": int(
                    os.environ.get("GOOGLE_API_RATE_LIMIT", "60")
                ),
                "concurrent_requests": int(
                    os.environ.get("GOOGLE_API_CONCURRENT", "10")
                ),
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        if not self.config["veo"]["output_bucket"]:
            validation_results["warnings"].append(
                "VEO_OUTPUT_BUCKET not set, using default"
            )

        # Check Jules configuration
        if not self.config["jules"]["github_token"]:
            validation_results["warnings"].append(
                "JULES_GITHUB_TOKEN not set, Jules integration disabled"
            )
...
class RateLimiter:
    ...
    async def acquire(self):
        ...
        async with self.semaphore:
            ...
            self.last_request_times = [
                t for t in self.last_request_times if now - t < 60
            ]
    ...
    **kwargs,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                f"Attempt {attempt + 1} failed: {str(e)}. Retrying in {delay + jitter:.2f}s"
            )
...
class AIServiceManager:
    ...
    async def call_with_fallback(
        self, service_name: str, primary_func, *args, **kwargs
    ) -> Dict[str, Any]:
