...
class PerformanceMonitor:
    ...
    async def _check_performance_targets(self):
        ...
        try:
            ...
            for agent_id, metrics in self.agent_metrics.items():
                if metrics.get('error_rate', 0) > self.performance_targets['error_rate_percent']:
                    alerts.append({
                ...
                if metrics.get('avg_execution_time', 0) > self.performance_targets.get(f"{metrics.get('agent_type')}_generation_ms", 10000):
                    alerts.append({
                        'type': 'execution_time_high',
                        'agent_id': agent_id,
                        'value': metrics['avg_execution_time'],
                        'target': self.performance_targets.get(f"{metrics.get('agent_type')}_generation_ms", 10000)
                    })
    ...
    async def _cleanup_old_metrics(self):
        ...
        try:
            cutoff_time = datetime.now(timezone.utc).timestamp() - (self.retention_hours * 3600)
            
            # Remove old entries from buffer
            while self.metrics_buffer and self.metrics_buffer[0].timestamp.timestamp() < cutoff_time:
    ...
    async def _get_active_agents(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {'agent_id': 'starri-orchestrator', 'type': 'orchestrator', 'status': 'online'},
    ...
    async def _calculate_agent_performance(self, agent_id: str) -> Optional[AgentPerformance]:
