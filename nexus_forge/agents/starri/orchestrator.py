...
class StarriOrchestrator:
        ...
        try:
            ...
            for step in range(max_thinking_steps):
                ...
                thought_result = await self.gemini_client.generate_content(
                    ...
                    generation_config={
                        "temperature": 0.7 if mode == ThinkingMode.DEEP_ANALYSIS else 0.4,
                ...
                current_thought = {
                    ...
                    "requires_more_thinking": self._check_needs_more_thinking(thought_content),
                    "timestamp": datetime.now(timezone.utc).isoformat()
                }
                ...
                if not current_thought["requires_more_thinking"] or current_thought["confidence"] > 0.95:
                    break
                
                # Prepare next iteration prompt
                thinking_prompt = self._prepare_continuation_prompt(thinking_chain, mode)
            ...
            self.metrics["average_thinking_time"] = (
                self.metrics["total_thinking_time"] / 
            ...
            if result["confidence"] > 0.8:
                self.cache.set(cache_key, result, timeout=3600, strategy=CacheStrategy.SEMANTIC)
    ...
    async def decompose_task(
        ...
        try:
            ...
            decomposition = self._parse_task_decomposition(thinking_result["conclusion"])
        ...
        try:
            ...
            try:
                await asyncio.wait_for(
                    monitoring_task,
                    timeout=decomposition.get("max_duration", 3600)
                )
    ...
    def _extract_confidence(self, thought_content: str) -> float:
        ...
        confidence_match = re.search(r'confidence[:\s]+(\d+\.?\d*)%?', thought_content.lower())
        if confidence_match:
        ...
        high_confidence_words = ["certain", "definitely", "clearly", "obviously", "undoubtedly"]
        ...
        medium_count = sum(1 for word in medium_confidence_words if word in thought_lower)
    ...
    def _check_needs_more_thinking(self, thought_content: str) -> bool:
        ...
        has_continuation = any(phrase in thought_lower for phrase in continuation_phrases)
    ...
    def _extract_quality_score(self, reflection_content: str) -> float:
        ...
        score_match = re.search(r'quality score[:\s]+(\d+\.?\d*)', reflection_content.lower())
        if score_match:
            return float(score_match.group(1))
        
        # Fallback: analyze sentiment of reflection
        positive_words = ["excellent", "effective", "comprehensive", "thorough", "insightful"]
    ...
    def _extract_improvements(self, reflection_content: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(indicator in line.lower() for indicator in 
                   ["could be improved", "improvement", "should", "recommendation", "suggest"]):
                capture_next = True
    ...
    def _parse_task_decomposition(self, conclusion: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "required_agents": list(set(
                cap for task in subtasks 
                for cap in task.get("required_capabilities", [])
            )),
            "execution_strategy": "parallel" if len(subtasks) > 3 else "sequential"
        }
    
    async def _validate_decomposition(self, decomposition: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for i, subtask in enumerate(decomposition.get("subtasks", [])):
            ...
            for capability in subtask.get("required_capabilities", []):
                if not any(capability in str(agents) for agents in self.agent_capabilities.values()):
    ...
    async def _execute_coordination_plan(
        ...
        try:
            ...
            results["metrics"]["total_time"] = time.time() - results["metrics"]["start_time"]
    ...
    async def _execute_single_task(
        ...
        if not assigned:
            raise AgentError(f"Failed to assign task {task_id} to agent {selected_agent}")
        ...
        while time.time() - start_time < max_wait:
            task_status = await self.coordination_client.client.table('tasks').select('*').eq(
                'task_id', task_id
            ).execute()
            
            if task_status.data and task_status.data[0]["status"] in ["completed", "failed"]:
    ...
    def _find_suitable_agents(self, required_capabilities: List[str]) -> List[str]:
        ...
        for agent_id, agent_info in self.registered_agents.items():
            ...
            if any(req_cap in str(agent_capabilities) for req_cap in required_capabilities):
    ...
    async def _monitor_workflow_execution(self, workflow_id: str):
        ...
        while workflow_id in self.active_workflows:
            try:
                ...
                workflow_status = await self.coordination_client.client.table('workflows').select('*').eq(
                    'workflow_id', workflow_id
                ).execute()
                ...
                tasks = await self.coordination_client.client.table('tasks').select('*').eq(
                    'workflow_id', workflow_id
                ).execute()
                ...
                for task in tasks.data:
                    if task["status"] == "running":
                        ...
                        if (datetime.now(timezone.utc) - started_at).seconds > 600:  # 10 minutes
    ...
    async def _reflect_on_execution(
        ...
        await self.knowledge_client.add_execution_pattern(
            ...
            success_rate=execution_results['metrics']['tasks_completed'] / 
            ...
            if execution_results['metrics']['tasks_completed'] + 
               execution_results['metrics']['tasks_failed'] > 0 
            else 0
        )
        ...
        while True:
            try:
                for agent_id in list(self.registered_agents.keys()):
                    ...
                    agent_state = await self.coordination_client.get_agent_state(agent_id)
                    if agent_state:
                        last_heartbeat = datetime.fromisoformat(
                            agent_state.get("last_heartbeat", "")
                        )
                        if (datetime.now(timezone.utc) - last_heartbeat).seconds > 300:  # 5 minutes
