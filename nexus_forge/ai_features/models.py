...
class TrainingDataSource(BaseModel):
    ...
    preprocessing: Dict[str, Any] = Field(default_factory=dict, description="Preprocessing config")
    validation_split: float = Field(0.2, ge=0, le=0.5, description="Validation split ratio")
...
class HyperParameters(BaseModel):
    ...
    attention_heads: Optional[int] = Field(None, gt=0, description="Number of attention heads")
    sequence_length: Optional[int] = Field(None, gt=0, description="Sequence length")
    
    # Advanced parameters
    gradient_clip_norm: Optional[float] = Field(None, gt=0, description="Gradient clipping norm")
...
class TrainingMetrics(BaseModel):
    ...
    train_accuracy: Optional[float] = Field(None, ge=0, le=1, description="Training accuracy")
    val_accuracy: Optional[float] = Field(None, ge=0, le=1, description="Validation accuracy")
    ...
    gpu_utilization: Optional[float] = Field(None, ge=0, le=100, description="GPU utilization %")
...
class ModelEvaluation(BaseModel):
    ...
    class_metrics: Dict[str, Dict[str, float]] = Field(default_factory=dict, description="Per-class metrics")
    ...
    custom_evaluations: Dict[str, Any] = Field(default_factory=dict, description="Custom evaluations")
...
class TrainingJob(BaseModel):
    ...
    base_model_id: Optional[str] = Field(None, description="Base model for transfer learning")
    
    # Data configuration
    data_sources: List[TrainingDataSource] = Field(..., description="Training data sources")
    ...
    # Metrics and results
    training_metrics: List[TrainingMetrics] = Field(default_factory=list, description="Training metrics")
    evaluation_results: Optional[ModelEvaluation] = Field(None, description="Final evaluation")
...
class PredictionModel(BaseModel):
    ...
    preprocessing_config: Dict[str, Any] = Field(default_factory=dict, description="Preprocessing config")
    postprocessing_config: Dict[str, Any] = Field(default_factory=dict, description="Postprocessing config")
    
    # Performance metrics
    evaluation_metrics: ModelEvaluation = Field(..., description="Model evaluation results")
    benchmark_results: Optional[Dict[str, float]] = Field(None, description="Benchmark test results")
    ...
    avg_inference_time_ms: float = Field(0.0, ge=0, description="Average inference time")
...
class PredictionRequest(BaseModel):
    ...
    batch_size: Optional[int] = Field(None, gt=0, description="Batch size for batch predictions")
...
class QualityMetrics(BaseModel):
    ...
    dimensional_scores: Dict[QualityDimension, float] = Field(
        default_factory=dict,
        description="Scores by quality dimension"
    )
    
    # Detailed metrics
    performance_metrics: Dict[str, float] = Field(default_factory=dict, description="Performance metrics")
    reliability_metrics: Dict[str, float] = Field(default_factory=dict, description="Reliability metrics")
    ...
    # Comparison data
    baseline_score: Optional[float] = Field(None, description="Baseline score for comparison")
    improvement_percent: Optional[float] = Field(None, description="Improvement from baseline")
...
class CoordinationPrediction(BaseModel):
    ...
    predicted_cpu_usage: float = Field(..., ge=0, le=100, description="Predicted CPU usage %")
    predicted_memory_usage: float = Field(..., ge=0, le=100, description="Predicted memory usage %")
    predicted_request_volume: int = Field(..., ge=0, description="Predicted request volume")
    
    # Performance predictions
    predicted_response_time_ms: float = Field(..., gt=0, description="Predicted response time")
    predicted_error_rate: float = Field(..., ge=0, le=1, description="Predicted error rate")
    predicted_throughput: float = Field(..., gt=0, description="Predicted throughput")
    
    # Coordination recommendations
    recommended_agent_count: int = Field(..., gt=0, description="Recommended agent count")
    ...
    uncertainty_bounds: Dict[str, Tuple[float, float]] = Field(
        default_factory=dict,
        description="Uncertainty bounds for predictions"
    )
