...
class VoiceInteractionHandler:
        ...
        try:
            ...
            config = speech_v1.RecognitionConfig(
                encoding=encoding_map.get(
                    audio_format, speech_v1.RecognitionConfig.AudioEncoding.WEBM_OPUS
                ),
    ...
    async def generate_audio_response(
        ...
        try:
            ...
            voice_config = self.voice_configs.get(
                f"starri_{emotion}", self.voice_configs["starri_default"]
            )
            ...
            audio_config = texttospeech_v1.AudioConfig(
                audio_encoding=audio_encoding_map.get(
                    format, texttospeech_v1.AudioEncoding.MP3
                ),
    ...
    async def create_streaming_session(
        self, session_id: str, user_id: str, language: str = "en-US"
    ...
    async def process_audio_stream(
        ...
        if not session:
            yield {"type": "error", "error": "No active streaming session"}
            return
    ...
    def _create_ssml(self, text: str, emotion: str) -> str:
        ...
        if emotion == "excited":
            ...
            for word in emphasis_words:
                text = text.replace(
                    f" {word} ", f' <emphasis level="strong">{word}</emphasis> '
                )
    ...
    async def generate_voice_feedback(
        self, feedback_type: str, context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        templates = feedback_templates.get(
            feedback_type, feedback_templates["thinking"]
        )
        import random
